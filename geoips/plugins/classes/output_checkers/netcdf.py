...
class NetcdfOutputCheckerPlugin(BaseOutputCheckerPlugin):
    """NetCDF Output Checker Plugin Class.

    Inherits from BaseOutputCheckerPlugin, which contains shared functionality among
    output_checker plugins. Responsible for comparing two NetCDF files
    ...
    def outputs_match(self, output_product, compare_product):
        ...
        try:
            ...
            log_with_emphasis(
                LOG.interactive, *[line for line in str(resp).split("\n")]
            )
            diffout += [
            ...
            for varname in compare_xobj.variables:
                logged_messages = self.log_object_diff_values(
    ...
    def log_object_diff_values(
        self, object1, object2, compare_key, log_function=LOG.info
    ):
        r"""
        ...
        Notes
        -----
        Only logs and returns messages for non-zero differences. If there are no
        non-zero differences, an empty list is returned.
    ...
    def call(self, compare_path, output_products):
        """Compare the "correct" NetCDF files found the list of current output_products.
...
PLUGIN_CLASS = NetcdfOutputCheckerPlugin
