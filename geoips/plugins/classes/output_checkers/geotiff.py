...
class GeotiffOutputCheckerPlugin(BaseOutputCheckerPlugin):
    """GeoTIFF Output Checker Plugin Class.

    Inherits from BaseOutputCheckerPlugin, which contains shared functionality among
    output_checker plugins. Responsible for comparing two GeoTIFF files
    ...
    def correct_file_format(self, fname: str) -> bool:
        ...
        """
        return Path(fname).suffix.lower() in self.VALID_EXTENSIONS
    ...
    def _filter_metadata(self, profile: dict) -> dict:
        ...
        return {
            ...
            if key.lower() not in self.IGNORED_METADATA_FIELDS
        }

    def _compare_metadata(self, output_profile: dict, compare_profile: dict) -> bool:
        ...
        """
        filtered_output = self._filter_metadata(output_profile)
        filtered_compare = self._filter_metadata(compare_profile)
        return filtered_output == filtered_compare

    def _compare_arrays(
        self, output_data: np.ndarray, compare_data: np.ndarray
    ) -> bool:
    ...
    def _read_geotiff(self, filepath: Path) -> tuple[np.ndarray, dict]:
    ...
    def _write_geotiff(self, filepath: Path, data: np.ndarray, profile: dict) -> None:
    ...
    def _create_noisy_data(
        self, data: np.ndarray, scale: float, rng: np.random.Generator
    ) -> np.ndarray:
    ...
    def outputs_match(self, output_product: str, compare_product: str) -> bool:
        ...
        if not all(map(self._validate_file_exists, (output_path, compare_path))):
            return False

        try:
            output_data, output_profile = self._read_geotiff(output_path)
            compare_data, compare_profile = self._read_geotiff(compare_path)

            if not self._compare_metadata(output_profile, compare_profile):
                self._log_mismatch(
            ...
            if not self._compare_arrays(output_data, compare_data):
                max_diff, mean_diff = self._calculate_difference_stats(
                    output_data, compare_data
                )
                self._log_mismatch(
    ...
    def call(self, compare_path: str, output_products: list[str]) -> int:
        ...
        """
        return self.compare_outputs(compare_path, output_products)

    def _setup_test_directory(self, test_data_dir: str) -> Path:
    ...
    def _get_source_tiff_path(self) -> Path:
        ...
        return Path(
            str(files("geoips") / "../tests/outputs/ahi.tc.WV.geotiff" / tif_name)
        )
    ...
    def _generate_test_files(
        ...
        -------
        tuple[Path, list[Path]]
            Tuple of (compare_file, [matched_file, close_mismatch_file,
            bad_mismatch_file]).
        """
        file_specs = [
            ...
            (
                "close_mismatch.tif",
                partial(
                    self._create_noisy_data,
                    scale=self.CLOSE_MISMATCH_NOISE_SCALE,
                    rng=rng,
                ),
            ),
            (
                ...
                partial(
                    self._create_noisy_data,
                    scale=self.BAD_MISMATCH_NOISE_SCALE,
                    rng=rng,
                ),
            ),
        ]
        ...
        for filename, transform in file_specs:
            ...
            self._write_geotiff(filepath, transformed_data, profile)
            generated_paths.append(filepath)

        compare_file, *test_files = generated_paths
        return compare_file, test_files

    def get_test_files_long(self, test_data_dir: str) -> tuple[str, list[str]]:
        ...
        savedir = self._setup_test_directory(test_data_dir)
        source_path = self._get_source_tiff_path()

        temp_compare = savedir / "compare.tif"
        shutil.copy(source_path, temp_compare)

        compare_data, profile = self._read_geotiff(temp_compare)
        compare_file, test_files = self._generate_test_files(
            savedir, compare_data, profile, rng
        )

        return str(compare_file), [str(path) for path in test_files]

    def perform_test_comparisons_long(
        self, compare_file: str, test_files: list[str]
    ) -> None:
        ...
        comparison_results = [
            self.outputs_match(test_file, compare_file) for test_file in test_files
        ]
...
PLUGIN_CLASS = GeotiffOutputCheckerPlugin
