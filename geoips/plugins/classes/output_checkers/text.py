...
class TextOutputCheckerPlugin(BaseOutputCheckerPlugin):
    ...
    def _print_rich_success(self, message: str):
    ...
    def _print_rich_error(self, title: str, *messages: str):
    ...
    def _print_rich_warning(self, title: str, *messages: str):
    ...
    def _print_rich_diff(self, diff_output: str, file1: str, file2: str):
        ...
        if USE_RICH and RICH_AVAILABLE:
            ...
            def hard_wrap_filename(fname):
                ...
                fname = str(fname)
                return "\n".join(
                    fname[i : i + width] for i in range(0, len(fname), width)
                )

            file1, file2 = hard_wrap_filename(file1), hard_wrap_filename(file2)
            table.add_row(file1, file2)
            console.print(
                Panel(table, title="Files Being Compared", border_style="yellow")
            )
    ...
    def _handle_overwrite_prompt(self, file1: str, file2: str) -> bool:
        """Handle the overwrite prompt workflow."""
        if not PROMPT_TO_OVERWRITE_COMPARISON_FILE:
            return False

        if self._prompt_user_for_overwrite(file1, file2):
            # Overwrite file1 with file2
            copy(file2, file1)

            success_msg = (
                f"File overwritten successfully! {file2} has replaced {file1}."
            )
            if USE_RICH and RICH_AVAILABLE:
                console.print(
                    Panel(
    ...
    def log_comparison_result(
        ...
        if PRINT_TO_CONSOLE and result.diff_output:
            self._print_rich_diff(result.diff_output, output_product, compare_product)

        # Handle overwrite prompt if files don't match

    def write_diff_file(self, file1, file2, output_file):
    ...
    def outputs_match(self, output_product, compare_product):
        ...
        if not result.matches:
            diff_file = self.get_out_diff_fname(compare_product, output_product)
            self.write_diff_file(output_product, compare_product, diff_file)

        self.log_comparison_result(result, output_product, compare_product, diff_file)
        if not result.matches:
            if self._handle_overwrite_prompt(compare_product, output_product):
                result.matches = True
    ...
    def call(self, compare_path, output_products):
        ...
        if len(output_products) > 1 and not isinstance(output_products, str):
            return all(
                self.compare_outputs(compare_path, output_product)
    ...
    def write_file_subset(self, lines, filename, percent_to_write, random_func=None):
        """Write a random subset of lines to a file."""
        if random_func is None:
            random_func = self.predictable_random.random
    ...
    def get_test_files(self, test_data_dir):
        ...
        test_files = [
            self.write_file_subset(content, str(save_dir / filename), percent)
