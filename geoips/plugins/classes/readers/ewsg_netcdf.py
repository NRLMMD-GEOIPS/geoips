...
class EwsgNetcdfReaderPlugin(BaseReaderPlugin):
    """Ewsg Netcdf reader plugin class."""

    interface = "readers"
    family = "standard"
    name = "ewsg_netcdf"

    # gvar_ch6 has only half scanlines of other channels (1,2,4), we temporally do not
    # read the ch6 in. We will modify this reader if gvar_ch6 is needed in the future.
    VARLIST = [
        "gvar_ch1",
        "gvar_ch2",
        "gvar_ch4",
    ...
    def call_single_time(
        ...
        geolocation_cache_backend : str
            * Specify to use either memmap or zarray to store pre-calculated geolocation
              data.
            * Not currently used, since these files have all required geolocation data
              in the netCDF file.
        ...
        for fname in fnames:
            ...
            # test hour/minute/second of "start_datetime" info from start_time (second
            # of the day). add scan_time info to determine the "end_datetime". scan_time
            # has time of scans for this input data file(one obs).  If end_time of this
            # data >24 hr, modify value of "day" from fname.  Note: scan_time units are
            # seconds from (start_time + time_adjust)
            ...
            # Goes VARiable (gvar) data is on GOES-12/13 - older GOES satellites were
            # gvissr ncdf_file.sensor_name is gvissr - I think that is a mistake on
            ...
            """
                # convert unit from degree to Kelvin for ch2, ch4, and ch6 (ch1 is in
                # unit of albedo)
...
PLUGIN_CLASS = EwsgNetcdfReaderPlugin
