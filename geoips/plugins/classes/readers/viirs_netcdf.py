...
The original reader (viirs_aotcimss_ncdf4_reader.py) was developed for geoips1,
...
on any one of DNB or IMG or MOD file.  In order to minimize duplicated execution
of VIIRS files, additional adjust of execution of the VIIRS files will be needed
...
class ViirsNetcdfReaderPlugin(BaseReaderPlugin):
    """Viirs Netcdf reader plugin class."""
    ...
    def _get_geolocation_metadata(self, orig_shape, fnames, xarray):
        """
    ...
    def required_chan(self, chans, varnames):
    ...
    def required_geo_chan(self, xarrays, xvarname):
        """Return True if required geolocation channel."""
        # for data_type in list(xarrays.keys()):
        #     if xvarname in list(xarrays[data_type].keys()):
        #         LOG.info('        SKIPPING %s geolocation channel %s, xarray
        #                           GEOLOCATION variable %s exists', data_type,
        #                           xvarname, xvarname)
        #         return False
        return True

    def required_geo(self, chans, data_type):
    ...
    def add_to_xarray(self, varname, nparr, xobj, dataset_masks, data_type, nparr_mask):
    ...
    def call(
        ...
        """
        # since fname is a LIST of input files, this reader needs additional adjustments
        # to read all files and put them into the XARRAY output (add one more array for
        # number of files)

        """
        fnames=['readers/data_viirs/20200916.081200.npp.viirs.viirs_npp_nasaearthdata_x.x.VNP02DNB.sdr.x.x.nc',\  # NOQA
            'readers/data_viirs/20200916.081200.npp.viirs.viirs_npp_nasaearthdata_x.x.VNP02IMG.sdr.x.x.nc',\  # NOQA
            'readers/data_viirs/20200916.081200.npp.viirs.viirs_npp_nasaearthdata_x.x.VNP02MOD.sdr.x.x.nc']  # NOQA

        fnames=['data_viirs/20200826.074800.npp.viirs.viirs_npp_nasaearthdata_x.x.VNP02DNB.sdr.x.x.nc',\  # NOQA
            'data_viirs/20200826.074800.npp.viirs.viirs_npp_nasaearthdata_x.x.VNP02IMG.sdr.x.x.nc',\  # NOQA
            'data_viirs/20200826.074800.npp.viirs.viirs_npp_nasaearthdata_x.x.VNP02MOD.sdr.x.x.nc']  # NOQA
        """
        ...
        for fname in sorted(fnames):
            ...
            if ncdf_file.platform == "JPSS-1":
                ...
                # Attribute still lists JPSS-1, but operational satellite name is
                # NOAA-20.
                xarrays[data_type].attrs["platform_name"] = "noaa-20"
            if ncdf_file.platform == "JPSS-2":
                # xarrays[data_type].attrs['platform_name']  = 'jpss-2'
                # Attribute still lists JPSS-2, but operational satellite name is
                # NOAA-21.
            ...
            if data_type in ["MOD", "IMG"]:
                ...
                vis_vars = self.VARLIST[vis_data_type]
            ...
            for var in list_vars:
                xvarname = var
                if var in self.xvarnames:
                    xvarname = self.xvarnames[var]
                ...
                if var in self.DNB_CHANNELS and self.required_chan(
                ...
                # requested.
                if (
                    var in self.BT_CHANNELS + self.REF_CHANNELS
                    and self.required_chan(chans, [btvarname])
                ...
                if (
                    var in self.BT_CHANNELS + self.REF_CHANNELS
                    and self.required_chan(chans, [radvarname])
                ...
                if var in self.REF_CHANNELS and self.required_chan(chans, [refvarname]):
                    LOG.info(
                ...
                for geo_target_data_type in geo_target_data_types:
                    if var in self.GVARLIST[geo_target_data_type] and self.required_geo(
                        ...
                        if not self.required_geo_chan(xarrays, xvarname):
                            continue
        ...
        if resample and area_def:
            ...
            for dtype in xarrays.keys():
                ...
                geo_metadata = self._get_geolocation_metadata(
                ...
                for varname in xarrays[dtype].variables.keys():
                    ...
                    if varname not in list(self.xvarnames.values()) + [
                        "latitude",
                        "longitude",
                    ]:
        ...
        for dtype in xarrays.keys():
            ...
            if "DNBRad" in list(xarrays[dtype].variables.keys()) and self.required_chan(
                chans, ["DNBRef"]
            ):
                with import_optional_dependencies(loglevel="info"):
...
PLUGIN_CLASS = ViirsNetcdfReaderPlugin
