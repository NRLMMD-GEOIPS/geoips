...
class AscatUhrNetcdfReaderPlugin(BaseReaderPlugin):
    """Ascat UHR Netcdf reader plugin class."""
    ...
    def read_byu_data(self, wind_xarray, fname):
        ...
        * attributes: source_name, platform_name, data_provider,
        ...
        if "L2B_filename" in wind_xarray.attrs and "metopa" in wind_xarray.L2B_filename:
            ...
            "L2B_filename" in wind_xarray.attrs and "metopb" in wind_xarray.L2B_filename
        ):
            wind_xarray.attrs["source_name"] = "ascatuhr"
            wind_xarray.attrs["platform_name"] = "metop-b"
        elif (
            "L2B_filename" in wind_xarray.attrs and "metopc" in wind_xarray.L2B_filename
        ):
            wind_xarray.attrs["source_name"] = "ascatuhr"
            wind_xarray.attrs["platform_name"] = "metop-c"
        elif (
            "SZF_filenames" in wind_xarray.attrs and "M02" in wind_xarray.SZF_filenames
        ):
            wind_xarray.attrs["source_name"] = "ascatuhr"
            wind_xarray.attrs["platform_name"] = "metop-a"
        elif (
            "SZF_filenames" in wind_xarray.attrs and "M01" in wind_xarray.SZF_filenames
        ):
            wind_xarray.attrs["source_name"] = "ascatuhr"
            wind_xarray.attrs["platform_name"] = "metop-b"
        elif (
            "SZF_filenames" in wind_xarray.attrs and "M03" in wind_xarray.SZF_filenames
        ):
            wind_xarray.attrs["source_name"] = "ascatuhr"
            wind_xarray.attrs["platform_name"] = "metop-c"
        elif (
        ...
        try:
            ...
            expected_yymmdd = os.path.basename(wind_xarray.source_file_names[0]).split(
                "_"
            )[4]
            ...
            # New filenames as of 20220826, no time, only YYYYMMDD, orbit num, and A or
            # D for ascending or descending
        ...
        if "wspeeds" in wind_xarray.variables:
            ...
            wind_xarray["wind_speed_kts"] = (
                wind_xarray["wind_speed_kts"] * self.MS_TO_KTS
            )
            ...
            wind_xarray["wind_speed_ambiguities_kts"] = (
                wind_xarray["wind_speed_ambiguities_kts"] * self.MS_TO_KTS
            )
        ...
        if "time" in wind_xarray.variables:
            ...
            if abs((native_min_time - numpy.datetime64(dt)) * 1e-9) > 86400:
                ...
                # expected, then assume the time array actually contains the correct
                # dates! Otherwise, time offset adjustments are needed
    ...
    def call(
        ...
        if (
            ...
            wind_xarrays = self.read_byu_data(wind_xarray, fname)
...
PLUGIN_CLASS = AscatUhrNetcdfReaderPlugin
