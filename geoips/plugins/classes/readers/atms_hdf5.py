...
"""Reader to read a granule NOAA ATMS SDR TBs in h5 format.
...
class AtmsHdf5ReaderPlugin(BaseReaderPlugin):
    """Atms Hdf5 reader plugin class."""
    ...
    def read_atms_file(self, fname, xarray_atms, metadata_only=False):
        ...
        if "ATMS-SDR_All" in fileobj["All_Data"].keys():  # for TB-data
            ...
            time_scan = self.convert_epoch_to_datetime64(tb_time)
        ...
        if "ATMS-SDR-GEO_All" in fileobj["All_Data"].keys():  # for geo-data
            ...
            #  get UTC time in datetime64 format required by geoips for each pixel
            time_scan = self.convert_epoch_to_datetime64(StartTime, use_shape=lat.shape)
            ...
            if not metadata_only:
                ...
                ingested["satellite_zenith_angle"] = data_select[
                    "SatelliteZenithAngle"
                ][()]
                ingested["satellite_azimuth_angle"] = data_select[
                    "SatelliteAzimuthAngle"
                ][()]
    ...
    def call(
        ...
        # if metadata_only is True:  # read-in datafiles first time if
        # 'metadata_only= True'
        xarray_atms = xr.Dataset()
        source_file_names = []
        for fname in fnames:
            xarray_atms = self.read_atms_file(
                fname, xarray_atms, metadata_only=metadata_only
            )
...
PLUGIN_CLASS = AtmsHdf5ReaderPlugin
