...
The MOD03 and MOD14 files have the geolocation (lat/lon) and sensor geometry
...
class ModisHdf4ReaderPlugin(BaseReaderPlugin):
    """Modis Hdf4 reader plugin class."""
    ...
    def parse_metadata(self, metadatadict):
        ...
        for ii in metadatadict.keys():
            ...
            if ii == "CoreMetadata.0":
                self.parse_core_metadata(metadata, metadatadict["CoreMetadata.0"])
            elif ii == "ArchiveMetadata.0":
                self.parse_archive_metadata(metadata, metadatadict["ArchiveMetadata.0"])
            elif ii == "StructMetadata.0":
                self.parse_struct_metadata(metadata, metadatadict["StructMetadata.0"])
            else:
                metadata[ii] = metadatadict[ii]
        return metadata

    def parse_struct_metadata(self, metadata, metadatastr):
        """Parse metadata struct."""
        pass

    def parse_core_metadata(self, metadata, metadatastr):
        """Parse core metadata."""
        ii = 0
    ...
    def parse_archive_metadata(self, metadata, metadatastr):
    ...
    def add_to_xarray(self, varname, nparr, xobj, cumulative_mask, data_type):
        """Add variable to xarray Dataset."""
        # cumulative_mask is not the best name for this variable. It is used for mask
        # info of field 'varname' so that is not actually a cumulative mask. It should
        # be a mask for each variable.
    ...
    def call(
        ...
        # fnames=['MYD021KM.A2021004.2005.061.NRT.hdf','MYD03.A2021004.2005.061.NRT.hdf']  # NOQA
        ...
        for fname in fnames:  # Loop MODIS files
            ...
            mf = SD(fname, SDC.READ)
            mf_metadata = self.parse_metadata(mf.attributes())
            ...
            if "MOD021KM" == cname or "MYD021KM" == cname:
                ...
                # corrections[chan][0] = (ems1km_um) effective central wavelength in
                # microns
            ...
            if cname == "MOD03" or cname == "MYD03":
                ...
                for datasettag in dataset_info.keys():  # loop the data_type
                    ...
                    for (
                        ...
                        scifile_names.keys()
                    ):  # loop variables associated with geo-info
                        ...
                        if datasettag == "QKM" or datasettag == "HKM":
                            ...
                            # x, y: 1-D array of coordinate in strictly ascending order
                            # kx, ky (integer,optional): degrees of the bivariate
                            # Spline.
                            #                                               Default is 3
            ...
            # elif ('MOD14' == cname or 'MYD14' == cname) and 'fire_mask' in chans:
                ...
                for currvar in scifile_names.keys():
                    ...
                    try:
                        ...
                        LOG.warning(
                            "SKIPPING %s does not exist in %s", currvar, datasettag
                        )
                        continue
                    attrs = select_data.attributes()  # get attributes of this field
                    data = select_data.get()  # get the all data of this field
                ...
                for ii in range(len(datapaths)):
                    ...
                    for jj in range(len(chanlocs)):
                        ...
                        for attrname in attrs:  # add attributes
                            xarrays[datasettag][channame + "Rad"].attrs[attrname] = (
                                attrs[attrname]
                            )
                        ...
                        if channame in corrections.keys():
                            ...
                            # By default, GeoIPS readers should store BT data internally
                            # in Kelvin.
        ...
        for dtype in xarrays.keys():
            ...
            if dtype != "METADATA" and "latitude" not in list(
...
PLUGIN_CLASS = ModisHdf4ReaderPlugin
