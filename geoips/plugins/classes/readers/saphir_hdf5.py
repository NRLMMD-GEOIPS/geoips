...
class SaphirHdf5ReaderPlugin(BaseReaderPlugin):
    """Saphir Hdf5 reader plugin class."""
    ...
    def call(
        ...
        # xarray_saphir['time']=xr.DataArray(pd.DataFrame(time_scan).astype(int).apply(pd.to_datetime,format='%Y%j%H%M'))  # NOQA
...
PLUGIN_CLASS = SaphirHdf5ReaderPlugin
