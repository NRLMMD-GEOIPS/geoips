...
class AbiL2NetcdfReaderPlugin(BaseReaderPlugin):
    ...
    def satpy_read(self, fname, chans):
    ...
    def xr_read(self, fname, chans=None):
        ...
        if chans:
            ...
            drop_vars = [
                x for x in ds_vars if x not in chans and x not in ["lat", "lon"]
            ]
    ...
    def call(
        ...
        try:
            metadata = self.get_metadata(fnames[0])
            end_metadata = self.get_metadata(fnames[-1])
        except KeyError:
        ...
        if (
            ...
            geoips_attrs["platform_name"] = metadata["file_info"][
                "platform_ID"
            ].replace("G", "goes-")
        ...
        try:
            geo = self.calculate_abi_geolocation(
                metadata,
                area_def,
                geolocation_cache_backend=geolocation_cache_backend,
        ...
        for fname in fnames:
            ...
            try:
                ...
                xarray = self.satpy_read(fname, chans)
            except ValueError:
                log.info("Attempting to load with xarray")
                xarray = self.xr_read(fname, chans)
...
PLUGIN_CLASS = AbiL2NetcdfReaderPlugin
