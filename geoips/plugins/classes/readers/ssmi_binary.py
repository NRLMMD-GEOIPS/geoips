...
class SsmiBinaryReaderPlugin(BaseReaderPlugin):
    """Ssmi Binary reader plugin class."""
    ...
    def call(
        ...
        if data_name != "def":
            LOG.info(
                "Warning: wrong SSMI SDR data type:  data_type={0}".format(data_name)
            )
            raise
    ...
    """
        ...
        while f1:  # loop scans of this file
            ...
            # set up time info for each B scan  (will set: A scantime = B scantime
            # later)
        ...
        # image xarray_lores.attrs['sample_distance_km'] = 25
        ...
        # MTIFs need to be "prettier" for PMW products, so 2km resolution for final
        # image xarray_85ab.attrs['sample_distance_km'] = 13
...
PLUGIN_CLASS = SsmiBinaryReaderPlugin
