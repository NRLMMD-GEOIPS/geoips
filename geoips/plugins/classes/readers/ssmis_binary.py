...
of reader is independent from the GEOIPS system whose environmental parameters must
...
class SsmisBinaryReaderPlugin(BaseReaderPlugin):
    """Ssmis Binary reader plugin class."""
    ...
    def call(
        ...
        for fname in fnames:
            xobjs_list += [
                self.read_ssmis_data_file(fname, metadata_only=metadata_only)
            ]

        final_xobjs = self.append_xarray_dicts(xobjs_list)

        return final_xobjs

    def append_xarray_dicts(self, xobjs_list):
    ...
    def read_ssmis_data_file(self, fname, metadata_only=False):
        ...
        for nn in range(nsdr):  # loop number of sdr data records
            ...
            try:
                ...
                # To my knowledge, the file end time is not included in the binary
                # header. The following offset was determined by taking the start/end
                # time listed in the file name, then dividing the total number of
                # seconds by the number of sensor data records, which came out to 45.5
                # seconds
            ...
            # initialization of imager variables
...
PLUGIN_CLASS = SsmisBinaryReaderPlugin
