...
class ViirsSdrHdf5ReaderPlugin(BaseReaderPlugin):
    """Viirs SDR Hdf5 reader plugin class."""
    ...
    def get_sci_data(self, hfile, sv_key):
        ...
        with h5py.File(hfile) as h5data:
            ...
            if "BrightnessTemperature" in h5data["All_Data"][band_key].keys():
                ...
                if (
                    ...
                    in h5data["All_Data"][band_key].keys()
                ):
                    bt_fac = h5data["All_Data"][band_key][
                        "BrightnessTemperatureFactors"
                    ][...]
            ...
            if "RadianceFactors" in h5data["All_Data"][band_key].keys():
                ...
                rad_cal = rad_raw
            # flag data
            flag_key = [k for k in h5data["All_Data"][band_key].keys() if "QF1_" in k][
                0
            ]
    ...
    def get_geo_data(self, bname, band_type, flist):
        ...
        if len(geo_list) == 0:
            ...
            LOG.warning("No GEO terrain corrected files found, reverting to ellipsoid.")
        ...
        geo_xr["satellite_zenith_angle"] = xr.DataArray(
            sat_zea, dims=("dim_0", "dim_1")
        )
        geo_xr["satellite_azimuth_angle"] = xr.DataArray(
            sat_azi, dims=("dim_0", "dim_1")
        )
    ...
    def call(
        ...
        dt_start = self.get_time(sorted_fnames[0])[0]
        dt_end = self.get_time(sorted_fnames[-1])[-1]
        plat_name = self.get_plat(sorted_fnames[0])
        meta_attrs = {
        ...
        vlist = list(chain.from_iterable(self.VARLIST.values()))
        trim_flist = [
            f for f in fnames for v in vlist if "SV" + v in os.path.basename(f)
        ]
        ...
        for scifile in trim_flist:
            ...
            LOG.info(
                "Parsing VIIRS file; band {}, filename {}".format(band_fname, fn_base)
            )
            band_type = band_fname[:3]
            band_spec = band_fname[2:]

            sci_xrt = self.get_sci_data(scifile, band_spec)

            # apply children xr to parents
            parent_key = [k for k, v in self.VARLIST.items() if band_spec in v][0]
            # update, append, or create xarray
            if parent_key in full_xr.keys():
                if band_spec + "Rad" in list(full_xr[parent_key].variables):
                    # append
                    geo_xrt = self.get_geo_data(fn_base, band_type, fnames)
        ...
        if bowtie:
            ...
            for k, dset in full_xr.items():
                ...
                for rad_name in rad_list:
                    ...
                    rad_bow, slat, slon = self.bowtie_correction(
                        rad_var, tmp_lat, tmp_lon
                    )
                ...
                for bt_name in bt_list:
                    ...
                    bt_bow, slat, slon = self.bowtie_correction(
                        bt_var, tmp_lat, tmp_lon
                    )
        ...
        if "DNBRef" in chans or chans is None:
            ...
            full_xr["DNB"]["DNBRef"] = xr.DataArray(
                lunar_ref_data, dims=("dim_0", "dim_1")
            )
...
PLUGIN_CLASS = ViirsSdrHdf5ReaderPlugin
