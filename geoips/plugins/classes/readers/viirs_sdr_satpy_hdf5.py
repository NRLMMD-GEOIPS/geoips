...
class ViirsSdrSatpyHdf5ReaderPlugin(BaseReaderPlugin):
    """Viirs SDR Satpy Hdf5 reader plugin class."""

    interface = "readers"
    family = "standard"
    name = "viirs_sdr_satpy_hdf5"
    ...
    def bowtie_correction(self, band, lat, lon):
        ...
        interp_nans = interp(
            *np.indices(rad_fold.shape)[:, rad_nan_flag], **{"workers": 4}
        )
        res_band[rad_nan_flag] = interp_nans

        return res_band, ord_lat.astype(np.float64), sort_lon.astype(np.float64)

    def check_valid_satpy_filename(self, filename):
        """Check if VIIRS SDR filename is supported by satpy reader.

        VIIRS SDR HDF5 files must include the creation time in filename after the start
        and end times, otherwise will not recognize the file.

        Parameters
        ----------
        filename : str
            Filename of the VIIRS SDR HDF5 file (basename, not full file path)

        Returns
        -------
        bool
            True if valid satpy filename
        """
        complete_filename_pattern = (
            r"^[A-Za-z0-9]{5}_[a-z]{1}\d{2}_d\d{8}_t\d{7}_e\d{7}_b\d{5}_c\d{15,21}"
            r"_[a-zA-Z0-9_]{5,10}\.h5$"
        )
        return bool(re.match(complete_filename_pattern, filename))

    def check_filename_missing_ctime(self, filename):
        """Check if VIIRS SDR filename is missing the creation time.

        VIIRS SDR HDF5 files must include the creation time in filename after the start
        and end times, otherwise will not recognize the file. If this function returns
        True, the filename will be supported by satpy if the creation time is added to
        the name.
        ...
        -------
        bool
            True if the filename contains the start and end times, but is missing the
            ctime
        """
        missing_ctime_filename_pattern = (
            r"^[A-Za-z0-9]{5}_[a-z]{1}\d{2}_d\d{8}_t\d{7}_e\d{7}_b\d{5}_[a-zA-Z0-9_]{5,10}"  # NOQA
            r"\.h5$"
        )
        return bool(re.match(missing_ctime_filename_pattern, filename))

    def add_creation_time_to_filename(self, filename):
        """Add a product creation time to an SDR file that is missing the creation time.

        We will use the end time in the filename as a placeholder for the creation time.
        ...
        -------
        str
            Filename with creation time inserted after d%Y%m%d_s%H%M%S_e%H%MS_bNNNNN.
        """
        split_fname = filename.split("_")
        creation_time = "c" + split_fname[2][1:] + split_fname[4][1:]
        return "_".join(split_fname[:-2] + [creation_time] + split_fname[-2:])

    def check_filenames_and_link(self, filepaths):
        """Attempt to link to cache directory with valid name if applicable/able.

        Parameters
        ----------
        filepaths : list
            Filepaths passed to reader

        Returns
        -------
        dict
            Lists of valid files, linked files, and invalid files
        """
        target_link_dir = os.path.join(
            gpaths["GEOIPS_DATA_CACHE_DIR_SHORTTERM_CALIBRATED_DATA"],
            "viirs_sdr_hdf5_reader",
        )
        checked = {"valid": [], "linked": [], "invalid": []}
        for filepath in filepaths:
            filename = Path(filepath).name
            if self.check_valid_satpy_filename(filename):
                # This filename is supported by satpy - no action required
                checked["valid"].append(filepath)
            elif self.check_filename_missing_ctime(filename):
                # This is a valid VIIRS SDR file, but is missing the creation time
                # Use the end time as the creation time, and link to GeoIPS cache dir
                new_filename = self.add_creation_time_to_filename(filename)
                if not Path(target_link_dir).exists():
                    Path(target_link_dir).mkdir(parents=True, exist_ok=True)
                linked_path = os.path.join(target_link_dir, new_filename)
                if not Path(linked_path).exists():
                    LOG.info("Linking %s as %s", filepath, linked_path)
                    Path(linked_path).symlink_to(filepath)
                checked["linked"].append(linked_path)
            else:
                # This filename is right out
                checked["invalid"].append(filepath)
        return checked
    ...
    def call(
        ...
        """
        # print("Reading")
        check_fnames = self.check_filenames_and_link(fnames)
        fnames = check_fnames["valid"] + check_fnames["linked"]
        ...
        if metadata_only:
            ...
            for lfile in check_fnames["linked"]:
                if Path(lfile).exists():
                    LOG.info("Unlinking %s", lfile)
                    Path(lfile).unlink()
            return tmp_dict

        # trim VARLIST based on channels requested
        if chans:
            tmp_vl = self.VARLIST.copy()
        ...
        for var in self.VARLIST:
            ...
            dataset_ids = [
                ...
                if idx["name"] in self.VARLIST[var]
            ]
            ...
            for d in dataset_ids:
                ...
                # bowtie correction
                if bowtie:
                    band_data, band_lat, band_lon = self.bowtie_correction(
                        tmp_ma, lat, lon
                    )
                else:
            ...
            tmp_attrs = tmp_scn[self.VARLIST[var][0]].attrs
            ...
            if var == "DNB":
                ...
                tmp_cal_params["moon_frac"] = (
                    ("dim_0", "dim_1"),
                    np.full(
                        tmp_cal_params[cal_params[0].removeprefix("dnb_")][1].shape,
                        0.5 * (1 + np.cos(phase_ang)),
                    ),
                )
            ...
            if var == "DNB":
                try:
                    ...
                    tmp_dask |= {
                        "DNBRef": (("dim_0", "dim_1"), lunarref_data),
                    }
...
PLUGIN_CLASS = ViirsSdrSatpyHdf5ReaderPlugin
