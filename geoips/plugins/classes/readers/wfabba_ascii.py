...
class WfabbaAsciiReaderPlugin(BaseReaderPlugin):
    """Wfabba Ascii reader plugin class."""
    ...
    def read_wfabba_header(self, wfabba_file):
        """Read WFABBA header."""
        with open(wfabba_file, "r") as f:
            ...
            while is_header:
                line = f.readline()
                if line[:3] == "###":
                    parsed_line = self.parse_header_line(line)
                    if parsed_line:
    ...
    def read_wfabba_text(self, wfabba_file):
        """Read WFABBA text from file wfabba_file into xarray Dataset."""
        xobj = xarray.Dataset()
        header_meta = self.read_wfabba_header(wfabba_file)
        text = np.genfromtxt(
            wfabba_file,
            delimiter=",",
    ...
    def call(
        ...
        """
        xarray_objs = []
        metadata = self.read_wfabba_header(fnames[0])
        end_metadata = self.read_wfabba_header(fnames[-1])
        geoips_attrs = {
        ...
        for i, fname in enumerate(fnames):
            ...
            wfabba_xobj = self.read_wfabba_text(fname)
...
PLUGIN_CLASS = WfabbaAsciiReaderPlugin
