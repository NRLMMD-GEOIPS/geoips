...
class SmosWindsNetcdfReaderPlugin(BaseReaderPlugin):
    """Smos Winds Netcdf reader plugin class."""
    ...
    def read_smos_data(self, wind_xarray, fname):
        ...
        * attributes: source_name, platform_name, data_provider,
        ...
        if "_110_" in fname:
            ...
            # everything (< 3) because there were large data gaps without, but the data
            # near land really is poor quality so must remove.
            wind_xarray["wind_speed_kts"] = wind_xarray["wind_speed"].where(
                wind_xarray["quality_level"] < 2
            )[0, :, :]
        else:
        ...
        wind_xarray["wind_speed_kts"] = xarray.DataArray(
            data=np.flipud(wind_xarray.wind_speed_kts) * self.MS_TO_KTS,
            name="wind_speed_kts",
    ...
    def call(
        ...
        for fname in fnames:
            ...
            # SMOS time is not read in correctly natively with xarray - must pass fname
            # so we can get time information directly from netCDF4.Dataset open
            ingested += [self.read_smos_data(wind_xarray, fname)]
...
PLUGIN_CLASS = SmosWindsNetcdfReaderPlugin
