...
class AbiNetcdfReaderPlugin(BaseAbiReaderPlugin):
    """ABI Netcdf reader plugin class."""
    ...
    def metadata_to_datetime(self, metadata):
    ...
    def call_single_time(
        ...
        for fname in fnames:
            ...
            try:
                with ncdf.Dataset(str(fname), "r") as df:
                    all_metadata[fname] = self._get_metadata(df, fname)
        ...
        sdt, edt = self.metadata_to_datetime(highest_md)
        ...
        xarray_obj.attrs["perspective_point_height"] = all_metadata[fname][
            "projection"
        ...
        if area_def:
            ...
            scene_id = (
                highest_md["file_info"]["dataset_name"].split("_")[1].split("-")[2]
            )
            if scene_id == "RadF":
        ...
        # Create list of all possible channels for the case where no channels were
        # requested
        all_chans_list = []
        ...
        if self_register:
            ...
            standard_metadata[adname] = self._get_geolocation_metadata(
            ...
            if not gvars[adname]:
                LOG.error(
                    ...
                    f"is: {self.DONT_AUTOGEN_GEOLOCATION}"
                )
                gvars[adname] = {}
        else:
            for res in ["LOW", "MED", "HIGH"]:
                ...
                try:
                    # Get just the metadata we need
                    standard_metadata[res] = self._get_geolocation_metadata(res_md[res])
                    fldk_lats, fldk_lons = self.get_latitude_longitude(
        ...
        for dsname in datavars.keys():
            ...
            if roi is None:
                ...
                for curr_res in standard_metadata.keys():
                    if (
                        standard_metadata[curr_res]["res_km"] * 1000.0 * roi_factor
                        > roi
                    ):
                        roi = (
                            standard_metadata[curr_res]["res_km"] * 1000.0 * roi_factor
    ...
    def read_netcdf_radiance(self, full_disk, line_inds, sample_inds, md, gvars):
        """Read data for a full channel's worth of files."""
        ...
        # Use with to open the data file for reading and automatically close once
        # complete
        with ncdf.Dataset(md["path"], "r") as df:
            if not full_disk:
                ...
                # Here we need to determine which indexes to read based on the size of
                # the input geolocation data.  We assume that the geolocation data and
                # the variable data cover the same domain, just at a different
                # resolution.
    ...
    def convert_radiance_to_reflectance(
        self, radiance, solar_zenith_angle, calibration_metadata, bad_data_mask
    ):
    ...
    def get_cached_reflectance(
        ...
        convert_radiance_to_reflectance to calculate the reflectances then stores to
        zarray.
        ...
        if Path(ref_cache).exists() or Path(cache_partial).exists():
            ...
            if chunk_size:
                ...
                chunks = None
            LOG.info("GETDATACACHE to %s", cache_partial)
            ref = self.convert_radiance_to_reflectance(
    ...
    def convert_radiance_to_brightness_temperature(
        self, radiance, calibration_metadata, bad_data_mask
    ):
    ...
    def get_cached_brightness_temperature(
        ...
        Loads zarray file if cache exists, otherwise calls
        convert_radiance_to_brightness_temperature to calculate the reflectances then
        stores to zarray.
        ...
        if Path(bt_cache).exists() or Path(cache_partial).exists():
            ...
            if chunk_size:
                ...
                chunks = None
            LOG.info("GETDATACACHE to %s", cache_partial)
            bt = self.convert_radiance_to_brightness_temperature(
    ...
    def get_data(
        ...
        if cache_data:
            rad_data, qf = self.get_cached_radiance(
                full_disk,
                line_inds.copy(),
                sample_inds.copy(),
                md,
            ...
            rad_data, qf = self.read_netcdf_radiance(
                full_disk, line_inds, sample_inds, md, gvars
            )
        data = {"Rad": rad_data}
        # Originally was using -1, but this is a uint8 and set to 255 for off of disk
        data["Rad"][np.where(qf == 255)] = self.BADVALS["Off_Of_Disk"]
        data["Rad"][np.where(qf == 1)] = self.BADVALS["Conditional"]
        ...
        data["Rad"][np.where(qf == 3)] = self.BADVALS["No_Value"]
        ...
        if ref:
            ...
            if not rad:
                ...
                radiance = data["Rad"]
            if cache_data:
                ref = self.get_cached_reflectance(
                    radiance,
        ...
        if bt:
            ...
            if cache_data:
                bt = self.get_cached_brightness_temperature(
                    radiance,
                    md["var_info"],
                    bad_data_mask,
                    cache_name_prefix=cache_name_prefix,
...
PLUGIN_CLASS = AbiNetcdfReaderPlugin
