...
class AmsuaMhsMirReaderPlugin(BaseReaderPlugin):
    """Amsua MHS MIR reader plugin class."""
    ...
    def call(
        ...
        for fname in fnames:
            ...
            """    ------  Notes  ------
        Read AMSU-B hdf files for 5 chan antenna temperature (AT) and associated EDRs
        ...
        """
            ...
            for chan in chans:
                if chan in self.AMSUA_CHANS or chan in self.MHS_CHANS:
                    chan_ind = self.CHAN_INDEX_MAP[chan]
                    chan_num = chan_ind + 1
                    if chan in self.AMSUA_CHANS:
            ...
            for ingest_xr in [xarray_amsua, xarray_mhs, xarray_amsua_mhs]:
                ...
                ingest_xr["satellite_zenith_angle"] = xr.DataArray(
                    var_all["LZ_angle"][()]
                )
                ...
                # These variables were previously always read in... leaving them
                # commented out as an reminder for what is in the file....
...
PLUGIN_CLASS = AmsuaMhsMirReaderPlugin
