...
class AmiNetcdfReaderPlugin(BaseReaderPlugin):
    """AMI Netcdf reader plugin class."""
    ...
    def latlon_from_lincol_geos(
        ...
        Uses geostationary projection (likely won't work with extended local area
        files).
        Equations and code from GEO-KOMPSAT-2A Level 1B Data User Manual.
        """
        ...
        if not os.path.isfile(fname):
            ...
            if geolocation_cache_backend == "memmap":
                ...
                nlat = np.memmap(
                    fname, mode="r", dtype=np.float32, offset=0, shape=shape
                )
                nlon = np.memmap(
    ...
    def _get_general_metadata(self, df):
    ...
    def _get_data_metadata(self, df):
    ...
    def _get_observation_metadata(self, df):
        ...
        for name in md_names:
            if hasattr(df, name):
                ...
                LOG.info(
                    "Warning! Observation metadata field missing: {0}".format(name)
                )
        return metadata

    def _get_projection_metadata(self, df):
    ...
    def _get_global_metadata(self, df):
        """Get file-level metadata."""
        metadata = {}
        metadata["general"] = self._get_general_metadata(df)
        metadata["data"] = self._get_data_metadata(df)
        metadata["observation"] = self._get_observation_metadata(df)
        metadata["projection"] = self._get_projection_metadata(df)
        return metadata
    ...
    def _get_metadata(self, df, fname):
        """
        ...
        metadata["global"] = self._get_global_metadata(df)
    ...
    # Get pixel data.
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    def get_data(self, gvars, fname, rad=False, ref=False, bt=False):
        ...
        if bt and not reflectance_chan:
            ...
            nu = self.CENTER_WAVENUMBERS[channel]
    ...
    def call_single_time(
        ...
        for fname in fnames:
            ...
            try:
                ...
                with ncdf.Dataset(str(fname), "r") as ds:
                    all_metadata[fname] = self._get_metadata(ds, fname)
        ...
        start_dt, end_dt = self.metadata_to_datetime(highest_md)
        ...
        # requested
        all_chans_list = []
        for chan_list in self.DATASET_INFO.values():
        ...
        if self_register:
            ...
            geo_metadata[adname] = self._get_geolocation_metadata(res_md[self_register])

            i = np.arange(0, geo_metadata[adname]["num_lines"], dtype="f")
            j = np.arange(0, geo_metadata[adname]["num_samples"], dtype="f")
            i, j = np.meshgrid(i, j)
            fldk_lats, fldk_lons = self.latlon_from_lincol_geos(
                self_register,
                j,
                i,
                geo_metadata[adname],
            ...
            for res in ["LOW", "MED", "HIGH"]:
                ...
                try:
                    geo_metadata[res] = self._get_geolocation_metadata(res_md[res])
        ...
        for chan, types in chan_info.items():
            ...
            data = self.get_data(gvars[gname], chan_md["path"], rad, ref, bt)
...
PLUGIN_CLASS = AmiNetcdfReaderPlugin
