...
class SgliL1BHdf5ReaderPlugin(BaseReaderPlugin):
    """Sgli L1 B Hdf5 reader plugin class."""
    ...
    def commit_to_dict(self, data_dict, key, tmp_xry):
    ...
    def rad_to_temp(self, rad, center_wl):
    ...
    def read_irs(self, infile, data_chans=ir_chans):
        ...
        with h5File(infile) as irs_h5:
            ...
            for k in data_chans:
                ...
                res_type = self.res_mapping[
                    int(raw_chan.attrs["Spatial_resolution"][0])
                ]
                ...
                temp = self.rad_to_temp(
                    toa_rad, raw_chan.attrs["Center_wavelength"][...] / 1000
                )
    ...
    def call(
        ...
        vnr_chans = self.vis_chans
        irs_chans = self.ir_chans

        if chans is not None:
            req_chans = set("Lt_" + i[:4] for i in chans)
            irs_chans = req_chans & self.ir_chans
            vnr_chans = req_chans & self.vis_chans
            polo_chans = req_chans & self.pol_chans
            if (
        ...
        for f in fnames:
            ...
            if "IRS" in f_basename:
                data_xarray = self.read_irs(f, data_chans=irs_chans)
                data_xr_dict = self.commit_to_dict(data_xr_dict, "IRS", data_xarray)
            elif "VNR" in f_basename:
                data_xarray = self.read_irs(f, data_chans=vnr_chans)
                data_xr_dict = self.commit_to_dict(data_xr_dict, "VNR", data_xarray)
            elif "POL" in f_basename:
                data_xarray = self.read_irs(f, data_chans=polo_chans)
                data_xr_dict = self.commit_to_dict(data_xr_dict, "POL", data_xarray)
            else:
                LOG.warning("No data type readable in file: {}".format(f_basename))
                raise ValueError(
                    "No matching file key for IRS, VNR, or POL in filename."
                )
...
PLUGIN_CLASS = SgliL1BHdf5ReaderPlugin
