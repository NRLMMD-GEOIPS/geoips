...
class AhiHsdReaderPlugin(BaseReaderPlugin):
    """AHI HSD reader plugin class."""
    ...
    def metadata_to_datetime(self, metadata, time_var="ob_start_time"):
    ...
    def get_latitude_longitude(
        ...
        if not Path(fname).exists():
            ...
            # Changing to use numexpr rather than numpy.  Appears to speed up each
            # statement by about five times.
            #
            ...
            # # Now I'm lost, but it seems to work.  Comes from the Himawari-8 users's
            # guide
            ...
            lats = ne.evaluate(
                ...
                "/ ((Rs-(Rs*cos(x)*cos(y)-Sd)/(cos(y)**2+ecc*sin(y)**2)*cos(x)*cos(y))**2"  # NOQA
                "+ ((Rs*cos(x)*cos(y)-Sd)"
    ...
    def _get_metadata_block_info(self, df):
        """
    ...
    def _get_metadata_block_01(self, df, block_info):
        """
        ...
        block_data["processing_center"] = (
            data[22:38].decode("ascii").replace("\x00", "")
        )
        ...
        block_data["total_header_length"] = np.frombuffer(data[70:74], dtype="uint32")[
            0
        ]
        ...
        block_data["file_format_version"] = (
            data[82:114].decode("ascii").replace("\x00", "")
        )
    ...
    def _get_metadata_block_02(self, df, block_info):
        """
    ...
    def _get_metadata_block_03(self, df, block_info):
        """
        ...
        block_data["earth_to_sat_radius"] = np.frombuffer(data[27:35], dtype="float64")[
            0
        ]
    ...
    def _get_metadata_block_04(self, df, block_info):
        """
        ...
        block_data["earthcenter_to_sat_dist"] = np.frombuffer(
            data[27:35], dtype="float64"
        )[0]
    ...
    def _get_metadata_block_05(self, df, block_info):
        """
        ...
        block_data["cent_wavelength"] = np.frombuffer(data[5:13], dtype="float64")[0]
        block_data["valid_bits_per_pixel"] = np.frombuffer(data[13:15], dtype="uint16")[
            0
        ]
        ...
        if block_data["band_number"] in range(7, 17):
            ...
            block_data["speed_of_light"] = np.frombuffer(data[83:91], dtype="float64")[
                0
            ]
    ...
    def _get_metadata_block_06(self, df, block_info):
        """
    ...
    def _get_metadata_block_07(self, df, block_info):
        """
    ...
    def _get_metadata_block_08(self, df, block_info):
        """
        ...
        block_data["block_num"] = np.frombuffer(data[0:1], dtype="uint8")[0]
        block_data["block_length"] = np.frombuffer(data[1:3], dtype="uint16")[0]
        block_data["center_scan_of_rotation"] = np.frombuffer(
            data[3:7], dtype="float32"
        )[0]
        block_data["center_line_of_rotation"] = np.frombuffer(
            data[7:11], dtype="float32"
        )[0]
        block_data["rotation_correction"] = np.frombuffer(data[11:19], dtype="float64")[
            0
        ]
        block_data["num_correction_info"] = np.frombuffer(data[19:21], dtype="uint16")[
            0
        ]

        start = 21
        block_data["line_num_after_rotation"] = np.empty(
            block_data["num_correction_info"]
        )
        ...
        for info_ind in range(0, block_data["num_correction_info"]):
            ...
            start += 10
        block_data["spare"] = (
            data[start : start + 40].decode("ascii").replace("\x00", "")
        )

        return block_data

    def _get_metadata_block_09(self, df, block_info):
        """
    ...
    def _get_metadata_block_10(self, df, block_info):
        """
    ...
    def _get_metadata_block_11(self, df, block_info):
        """
    ...
    def _get_metadata(self, df, **kwargs):
        """Gather metadata for the data file and return as a dictionary."""
        metadata = {}
        # Get metadata block info
        try:
            block_info = self._get_metadata_block_info(df)
        except IOError as resp:
            raise IOError(resp)

        # Read all 12 blocks
        metadata["block_01"] = self._get_metadata_block_01(df, block_info)
        metadata["block_02"] = self._get_metadata_block_02(df, block_info)
        metadata["block_03"] = self._get_metadata_block_03(df, block_info)
        metadata["block_04"] = self._get_metadata_block_04(df, block_info)
        metadata["block_05"] = self._get_metadata_block_05(df, block_info)
        metadata["block_06"] = self._get_metadata_block_06(df, block_info)
        metadata["block_07"] = self._get_metadata_block_07(df, block_info)
        metadata["block_08"] = self._get_metadata_block_08(df, block_info)
        metadata["block_09"] = self._get_metadata_block_09(df, block_info)
        metadata["block_10"] = self._get_metadata_block_10(df, block_info)
        metadata["block_11"] = self._get_metadata_block_11(df, block_info)
        ...
        metadata["sensor"] = "AHI"
        # Make accessable to parent classes that don't know the structure of our
        # metadata
    ...
    def _check_file_consistency(self, metadata):
        """
        ...
        # Times are incomparable using this field, but are compared below using
        # ob_timeline.
        start_dates = [
            int(metadata[fname]["block_01"]["ob_start_time"])
        ...
        #   ob_timeline: Provides HHMM for each image, so should be the same for all
        #   files from thes same image.
        ...
        for block in members_to_check.keys():
            for field in members_to_check[block].keys():
                member_vals = [
                    metadata[fname][block][field] for fname in metadata.keys()
                ]
    ...
    def sort_by_band_and_seg(self, metadata):
    ...
    def call_single_time(
        ...
        cache_chunk_size : int
            * Specify chunk size if using zarray to store pre-calculated geolocation
              data.
        resource_tracker: geoips.utils.memusg.memusg_tracker.PidLog object
            ...
            * The PidLog.track_resource_usage method allows us to snapshot the memory
              usage for the PID associated with the geoips call. The time and stats of
              the snapshot are recorded, and can be accessed using the
        ...
        for fname in fnames:
            ...
            if chans:
                ...
                for chan in chans:
                    currchan = (
                        chan.replace("BT", "").replace("Ref", "").replace("Rad", "")
                    )
            ...
            try:
                with open(fname, "rb") as file_stream:
                    all_metadata[fname] = self._get_metadata(file_stream)
            except IOError as resp:
                LOG.exception("BAD FILE %s skipping", resp)
                if ".bz2" in fname:
                    LOG.exception(
                        "BAD FILE %s must be bunzip2ed prior to processing", resp
                    )
                continue
            if metadata_only:
        ...
        if not self._check_file_consistency(all_metadata):
        ...
        start_dt = self.metadata_to_datetime(
            highest_md, time_var="ob_start_time"
        )  # Assume same for all
        end_dt = self.metadata_to_datetime(
        ...
        xarray_obj.attrs["platform_name"] = highest_md["block_01"][
            "satellite_name"
        ].lower()
        ...
        # If one or more channels are missing a segment that other channels have, add it
        # in as "None". This will be set to bad values in the output data
        ...
        all_chans_list = []
        for chl in list(self.ALL_CHANS.values()) + list(self.ALL_GVARS.values()):
            all_chans_list += chl
        ...
        # This saves us from having very slightly different solar angles for each
        # channel. Loop over resolutions and get metadata as needed
        # for res in ['HIGH', 'MED', 'LOW']:
        if self_register:
            LOG.info("")
            LOG.info("Getting geolocation information for adname %s.", adname)
            gmd = self._get_geolocation_metadata(res_md[self_register])
            ...
            for res in ["LOW", "MED", "HIGH"]:
                ...
                try:
                    gmd = self._get_geolocation_metadata(res_md[res])
                    fldk_lats, fldk_lons = self.get_latitude_longitude(
                        gmd,
                        self.BADVALS,
                        area_def,
        ...
        if geolocation_only:
            ...
            for dsname in gvars.keys():
                ...
                for varname in gvars[dsname].keys():
                    geo_xarrays[dsname][varname] = xarray.DataArray(
                        gvars[dsname][varname]
                    )
            return geo_xarrays
            ...
            if not datavars[ds]:
                ...
                for varname in datavars[ds].keys():
                    self.set_variable_metadata(
                        xarray_obj.attrs, band_metadata, ds, varname
                    )
                    datavars[ds][varname] = np.ma.masked_less(
                        datavars[ds][varname], -999.1
                    )
                    if (
                        satellite_zenith_angle_cutoff
        ...
        for dsname in datavars.keys():
            ...
            #     max(xobj.area_definition.pixel_size_x,
    ...
    def set_variable_metadata(self, xobj_attrs, band_metadata, dsname, varname):
        """
    ...
    def get_data(self, md, gvars, rad=False, ref=False, bt=False, zoom=1.0):
        """Read data for a full channel's worth of files."""
        # Coordinate arrays for reading
        # Unsure if Lines can ever be None, but the test below was causing an error due
        # to testing the truth value of an entire array.  May need to implement test
        # again here to ensure that gvars['Lines'] is actually something, but testing
        # this method for now.
        ...
        if "Lines" in gvars and "Samples" in gvars:
            ...
            # All required line numbers (used for determining which files to even look
            # at)
        ...
        # count_badval. To fis this, we can find the count value at which radiances
        # become less than or equal to zero. Any counts above that value are bad.
        ...
        # Note the weird memory manipulation to save memory when radiances are not
        # requested
        ...
        for val in data.values():
            ...
            val[root_inds] = self.BADVALS["Root_Test"]
            val[outrange_inds] = self.BADVALS["Out_Of_Valid_Range"]
            val[offdisk_inds] = self.BADVALS["Off_Of_Disk"]
            val[error_inds] = self.BADVALS["Error"]

        return data


PLUGIN_CLASS = AhiHsdReaderPlugin
