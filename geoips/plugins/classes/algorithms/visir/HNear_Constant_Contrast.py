...
class HighNearConstantContrastAlgorithmPlugin(BaseAlgorithmPlugin):
    """High Near Constant Contrast algorithm plugin class."""

    interface = "algorithms"
    family = "list_numpy_to_numpy"
    name = "high_near_constant_contrast"

    def gain_func(self, angle):
    ...
    def call(
        ...
        if len(arrays) != 4:
            raise ValueError(
                "Incongruent shape, please pass "
                "data,solar_zenith_angle,lunar_zenith_angle"
            )
...
PLUGIN_CLASS = HighNearConstantContrastAlgorithmPlugin
