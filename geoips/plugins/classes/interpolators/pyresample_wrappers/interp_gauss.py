...
class InterpGaussInterpolatorPlugin(BaseInterpolatorPlugin):
        ...
        **kwargs,
        ...
        if array_num is not None:
            ...
            if drop_nan is True:
                ...
                goodinds = (
                    ~input_xarray["longitude"][:, :, array_num].isnull() & goodinds
                )
            for varname in varlist:
                if drop_nan is True:
                    # Cumulative mask over all channels. What could go wrong?
                    goodinds = (
                        ~input_xarray[varname][:, :, array_num].isnull() & goodinds
                    )
        ...
        if drop_nan is True:
            ...
            for var in vars_to_interp:
                final_vars_to_interp += [
                    var.where(goodinds, drop=True).to_masked_array()
                ]
...
PLUGIN_CLASS = InterpGaussInterpolatorPlugin
