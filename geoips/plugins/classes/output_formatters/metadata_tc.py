...
class MetadataTcOutputFormatterPlugin(BaseOutputFormatterPlugin):
    ...
    def call(
        ...
        return self.output_tc_metadata_yaml(
            metadata_yaml_filename,
            area_def,
            xarray_obj,
    ...
    def update_sector_info_with_data_times(self, sector_info, xarray_obj):
    ...
    def update_sector_info_with_area_def_info(self, sector_info, area_def):
    ...
    def update_sector_info_with_data_attribution(self, sector_info, xarray_obj):
        """Update sector info with data attribution, for YAML metadata output."""
        for attr_key in ["data_attribution", "data_provider"]:
            if attr_key in xarray_obj.attrs:
                sector_info[attr_key] = xarray_obj.attrs[attr_key]
        return sector_info

    def update_sector_info_with_coverage(
        self, sector_info, product_name, xarray_obj, area_def, output_dict
    ):
        ...
        for covg_func_type, coverage_checker_plugin_name in covg_func_types.items():
            ...
            covg_varname = covg_args[covg_func_type].pop(
    ...
    def output_tc_metadata_yaml(
        ...
        sector_info = self.update_sector_info_with_coverage(
            sector_info,
        ...
        sector_info = self.update_sector_info_with_area_def_info(
            sector_info,
            area_def,
        )

        sector_info = self.update_sector_info_with_data_times(
            sector_info,
            xarray_obj,
        )

        sector_info = self.update_sector_info_with_data_attribution(
            sector_info,
            xarray_obj,
        )
...
PLUGIN_CLASS = MetadataTcOutputFormatterPlugin
