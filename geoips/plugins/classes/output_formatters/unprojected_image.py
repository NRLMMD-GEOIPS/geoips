...
class UnprojectedImageOutputFormatterPlugin(BaseOutputFormatterPlugin):
    ...
    def call(
        ...
        if savefig_kwargs is None:
            # Default to no arguments, empty dictionary.  Will result in masked
            # background
            savefig_kwargs = {}
            # black background
        ...
        # This needs to be done as output_formatter_kwargs can only be specified as
        # strings (to my knowledge at least)
        ...
        for slice_idx, slice_data in enumerate(slices):
            ...
            for fname in output_fnames:
                if is_3d:
                    # This is generic for overcast data, on the order of (level) * 0.5
                    # km.
                    # I.e. if level == 5, lvl_str == '02_50.'. This is an easy way to
                    # denote what height each image corresponds to, though I'm not sure
                    # if we should hardcode this string here. Maybe create a new
                    # filename formatter for that data or we can add a new argument to
                    # this plugin which handles fname conventions for 3D data. Such as a
                    # function which produces a pre-prended string to the corresponding
                    # fname.
...
PLUGIN_CLASS = UnprojectedImageOutputFormatterPlugin
