...
class TcSectorFileParserSectorMetadataGeneratorPlugin(
    BaseSectorMetadataGeneratorPlugin
):
    """TC Sector File Parser Sector Metadata generator plugin class."""

    interface = "sector_metadata_generators"
    family = "tc"
    name = "tc_sector_file_parser"

    def call(self, trackfile_name):
        """TC trackfile parser for flat text sectorfiles containing current active storms.  # NOQA
        ...
        for line in flatsf_lines:
            if not line.strip():
                continue
            curr_fields = self.parse_flat_sectorfile_line(
    ...
    def parse_flat_sectorfile_line(
        self, line, source_filename, parser_name="flat_sectorfile_parser"
    ):
    ...
    def NSEW_to_float(self, lat_lon_val):
    ...
    def get_storm_year(self, storm_basin, current_month, current_year):
...
PLUGIN_CLASS = TcSectorFileParserSectorMetadataGeneratorPlugin
