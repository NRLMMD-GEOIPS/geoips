...
class BdeckParserSectorMetadataGeneratorPlugin(DeckSectorMetaGeneratorPlugin):
    ...
    def call(self, trackfile_name, allowed_aid_types=None):
        ...
        storm_start_datetime_from_first_line_of_current_deck_file = (
            self.get_storm_start_datetime_from_bdeck_entry(flatsf_lines)
        )
        ...
        final_storm_name = self.get_final_storm_name_bdeck(
            flatsf_lines, tc_year, trackfile_name, is_archived
        )
        invest_number = self.get_invest_number_bdeck(flatsf_lines, is_archived)
        ...
        for line in flatsf_lines:
            try:
                curr_fields = self.parse_bdeck_line(
                    line,
    ...
    def get_invest_number_bdeck(self, deck_lines, is_archived):
        ...
        for line in deck_lines:
            try:
                fields = self.parse_bdeck_line(line)
            except ValueError:
                if is_archived:
                    LOG.warning("Skipping bad/poorly formatted line")
                else:
...
PLUGIN_CLASS = BdeckParserSectorMetadataGeneratorPlugin
