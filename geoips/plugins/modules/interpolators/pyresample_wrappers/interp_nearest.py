...
# required for pluginify to register this plugin
interface = "interpolators"
family = "2d"
name = "interp_nearest"


class InterpNearestInterpolatorPlugin(BaseInterpolatorPlugin):
    ...
    def get_final_roi(self, xarray_obj, area_def):
    ...
    def _pre_call(self, data=None, *args, _obp_initiated=False, **kwargs):
        """Move area_def from kwargs to the positional data slot.

        OBP injects area_def via globals kwargs as a keyword argument.
        Since interp_nearest.call() has area_def as its first positional
        parameter (filled by data in _invoke), having it in both places
        causes "multiple values for argument 'area_def'".  Pop it from
        kwargs; if data is None, use the popped value as data.
        """
        if "area_def" in kwargs:
            area_def_kw = kwargs.pop("area_def")
            if data is None:
                data = area_def_kw
    ...
    def call(
        self, area_def, input_xarray, output_xarray, varlist, array_num=None, **kwargs
    ):
        ...
        data_box_definition = get_data_box_definition(
...
PLUGIN_CLASS = InterpNearestInterpolatorPlugin
