...
class FciNetcdfReaderPlugin(BaseReaderPlugin):
    """FCI Netcdf reader plugin class."""
    ...
    def get_geolocation_metadata(self, xarray_attrs):
    ...
    def get_latitude_longitude(
        ...
        if not Path(fname).exists():
            ...
            # used however they are not recognized by flake8 linter under
            # numexpr.evaluate()
            # Constants
            pi = np.pi
            ...
            if geolocation_cache_backend == "memmap":
                ...
                # NOTE zarr does NOT have a close method, so you can NOT use with
                # context.
    ...
    def get_standard_geoips_attrs(self, file_attrs, area_def):
        ...
        if "TRAIL" in all_attrs["mtg_name"]:
            ...
            # time coverage start/end attr in the file metadata, use the
            # date_time_position attr, which is consistent across all files.
    ...
    def load_metadata_single_file(self, fci_file, group=None, variable=None):
    ...
    def get_ncattrs(self, fci_file, group=""):
        """Lightweight function to quickly return the attrs for a group in a netCDF file."""  # NOQA
        with ncdf.Dataset(fci_file, "r") as ds:
            attrs = {x: ds[group].getncattr(x) for x in ds[group].ncattrs()}
        return attrs

    def avail_chans_in_file(self, fci_file):
    ...
    def final_calibrated_variable_name(self, ncvar_name, chan_map=CHAN_MAP):
    ...
    def scene_to_xarray(self, fci_files, geoips_chans, metadata=None, area_def=None):
        ...
        geoips_chans : list
            ...
            'GeoIPS nomenclature' is the common variable mapping used in our
            geostationary readers. See 'BAND_MAP' variable for an example of these
            channels. Allows us to read in radiances, reflectances, or brightness
            temperatures even though the variable in the dataset itself (such as
            'vis_04') does not provide that information.
        ...
        for gchan in geoips_chans:
            ...
            # I.e. B01Rad and B01Ref. Rename these data queries to the GeoIPS variable
            # name so we don't have conflicts.
        ...
        geo_meta = self.get_geolocation_metadata(xr.attrs)
        gvars = get_geolocation(
            xr.start_time, geo_meta, lats, lons, geo_meta["BADVALS"]
        )
        ...
        for dkey in set(geoips_chans + list(gvars)):
            ...
            if dkey.endswith("Rad"):
                ...
                xr[dkey].data = (
                    xr[dkey].data * self.RADIANCE_CONVERSION_COEFFICIENTS[dkey]
                )
        ...
        if not metadata:
            metadata = self.get_standard_geoips_attrs(
                self.load_metadata_single_file(fci_files[0], group="data"), area_def
            )
    ...
    def convert_radiance_to_bt(
        self, variable_dataset, line_inds=None, sample_inds=None
    ):
        """Convert effective radiance to effective brightness temperature.

        The effective brightness temperature of a surface is the temperature of a
        spatially uniform blackbody that emits the equivalent amount of radiant energy
        as the surface within a spectral band characterized by the spectral response
        function of the instrument. Given the band-average spectral radiance per
        wavenumber Lv, i.e. the effective radiance, the effective brightness temperature
        T_eff can be approximated as follows:
        ...
        Constants C1 = 2hc2 and C2 = hc/k are radiation constants where c, h, and k are
        the speed of light, Planck, and Boltzmann constant, respectively.
        ...
        radiance_to_bt_conversion_coefficient_b contain the conversion coefficients a
        and b for IR channels, respectively. They are set to the _FillValue for VNIR
        channels.

        The variables radiance_to_bt_conversion_constant_c1
        and radiance_to_bt_conversion_constant_c2 contain the
        constants c1 and c2 for IR channels. Note that the values given in the dataset
        are c1=2·1011·hc2 =1.19104282E-05 and c2=100 hc/k=1.43877513 due to unit
        conversions.
    ...
    def compute_sun_earth_distance(self, start_datetime, end_datetime):
    ...
    def convert_radiance_to_reflectance(
        ...
        if earth_sun_distance_km is None:
            d_t = self.compute_sun_earth_distance(start_datetime, end_datetime)  # NOQA
        else:
    ...
    def read_with_xarray(
        ...
        for gchan in geoips_chans:
            ...
            with xarray.open_mfdataset(
                fci_files, group=f"/data/{rchan}/measured"
            ) as df:
                if "latitude" not in dset:
                    ...
                    geolocation_metadata = self.get_geolocation_metadata(metadata)
                ...
                if "Rad" in gchan:
                    ...
                    calibrated = eff_rad * self.RADIANCE_CONVERSION_COEFFICIENTS[gchan]
                elif "Ref" in gchan:
                    calibration_type = "Reflectance"
                    calibrated = self.convert_radiance_to_reflectance(
                        df,
                        dataset_name=rchan,
                    ...
                    calibrated = self.convert_radiance_to_bt(
                        df,
                        line_inds=gvars.get("Lines"),
                        sample_inds=gvars.get("Samples"),
                    )
    ...
    def read_fci_netcdf_files(
        ...
        avail_vars = self.avail_chans_in_file(fci_files[0])
        if not chans:
            # If chans is None, set chans to the GeoIPS band names for each variable
            # found in the FCI files.
            chans = [self.CHAN_MAP[x] for x in avail_vars]
        ingested = {}
        for fci_file in fci_files:
            if "TRAIL" in fci_file:
                # Use the TRAIL file if possible to
                standard_meta = self.get_standard_geoips_attrs(
                    self.load_metadata_single_file(fci_file, group="data"), area_def
                )
                break
        else:
            standard_meta = self.get_standard_geoips_attrs(
        ...
        if metadata_only:
            return ingested
        # Add projection information to top-level metadata, only available in actual
        # data files.
        ...
        # This is probably OK for now when self-registering (makes things easier as
        # satpy returns all data at the same size of the lowest available resolution)
        if self_register:
            adname = "FULL_DISK"
            if use_satpy:
                ingested[adname] = self.scene_to_xarray(
                    fci_files, chans, metadata=standard_meta
                )
            else:
                ingested[adname] = self.read_with_xarray(
                    fci_files,
                    chans,
        ...
        # Otherwise read in chans with similar resolutions to maintain resolution
            ...
            for res, rchans in self.DATASET_INFO.items():
                # Find the indices of variables that correspond to the current
                # resolution
                read_idxs = np.array([self.BAND_MAP.get(x, x) in rchans for x in chans])
                if not any(read_idxs) and not metadata_only:
                    LOG.info("No channels to read at %s resolution", res)
                    continue
                # Use those indices to retrieve the correct 'geoips' variables that
                # we'll need to read.  Cast np.str to str.
                read_geoips_chans = [str(x) for x in np.array(chans)[read_idxs]]
                LOG.info(
                    "Reading in following chans: %s: %s",
                    res,
                    " ".join(read_geoips_chans),
                )
                # Ingest the returned xarray dataset to the current resolution of the
                # xarray_dictionary
                if use_satpy:
                    ingested[res] = self.scene_to_xarray(
                        fci_files,
                        read_geoips_chans,
                        metadata=standard_meta,
                        area_def=area_def,
                    ...
                    ingested[res] = self.read_with_xarray(
                        fci_files,
                        read_geoips_chans,
                        metadata=standard_meta,
                        area_def=area_def,
    ...
    def call(
        ...
        if metadata_only:
            ...
            # overall start and end time across the files, so use that if we can.
            # Otherwise take the first file in the list.
            trail_files = []
...
PLUGIN_CLASS = FciNetcdfReaderPlugin
