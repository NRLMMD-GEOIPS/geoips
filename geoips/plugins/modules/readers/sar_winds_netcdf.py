...
class SarWindsNetcdfReaderPlugin(BaseReaderPlugin):
    """SAR Winds Netcdf reader plugin class."""
    ...
    def read_sar_data(self, wind_xarray):
        ...
        * attributes: source_name, platform_name, data_provider,
        ...
        rcm_attribution = [
            "RADARSAT Constellation Mission Imagery Copyright Government of Canada 2025",  # NOQA
        ...
        if (
            ...
            "platform" in wind_xarray.attrs and wind_xarray.platform.lower() == "rcm-1"
        ):
            ...
            "platform" in wind_xarray.attrs and wind_xarray.platform.lower() == "rcm-2"
        ):
            ...
            "platform" in wind_xarray.attrs and wind_xarray.platform.lower() == "rcm-3"
        ):
        ...
        # if "acknowledgment" in wind_xarray.attrs and "NOAA" in wind_xarray.acknowledgment:  # NOQA
        ...
        wind_xarray["wind_speed_kts"] = wind_xarray["sar_wind"] * self.MS_TO_KTS
        ...
        # wind_xarray['time'] = wind_pandas['acquisition_time'][:, 5, :].to_xarray().transpose()  # NOQA
        ...
        if (
            ...
            for fname in fnames:
                ...
                if columns == wind_xarray.columns:
                    wind_xarrays += self.read_sar_data(wind_xarray)
                else:
...
PLUGIN_CLASS = SarWindsNetcdfReaderPlugin
