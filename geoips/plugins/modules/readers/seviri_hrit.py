...
class SeviriHritReaderPlugin(BaseReaderPlugin):
    ...
    def compare_dicts(self, d1, d2, skip=None):
    ...
    def get_top_level_metadata(self, fnames, sect):
    ...
    def _get_geolocation_metadata(self, prologue, file_metadata):
    ...
    def countsToRad(self, counts, slope, offset):
        """Convert counts to rad."""
        rad = np.full_like(counts, -999.0)
        rad[counts > 0] = offset + (slope * counts[counts > 0])
        return rad

    def radToRef(self, rad, sun_zen, platform, band):
        """Convert Rad to Ref."""
        irrad = self.VIS_CALIB[platform][band]
        ref = np.full_like(rad, -999.0)
        # 0 to 1 rather than 0 to 100
        ref[rad > 0] = rad[rad > 0] / irrad
        # DO NOT REMOVE THIS STEP ALTOGETHER!!!!  Just take out the solar zenith
        # correction part. Previously, solar zenith correction was being applied twice,
        # then we were off by factor of pi/irrad. Now we should be good!
    ...
    def radToBT(self, rad, platform, band):
        """Convert rad to BT."""
        c1 = 1.19104e-05
        c2 = 1.43877
        wn = self.IR_CALIB[platform][band]["wn"]
        a = self.IR_CALIB[platform][band]["a"]
        b = self.IR_CALIB[platform][band]["b"]
    ...
    def call_single_time(
        ...
        # information)
        xarray_obj.attrs = self.get_top_level_metadata(fnames, area_def)
            ...
            if df.file_type == "prologue":
                ...
                for poly in df.prologue["satelliteStatus"]["orbit"]["orbitPolynomial"]:
                    if dt <= poly["endTime"] and dt >= poly["startTime"]:
                        ...
                        ycoef = poly["Y"]
                        zcoef = poly["Z"]
                        x = self.calculate_chebyshev_polynomial(xcoef, st, et, dt)
                        y = self.calculate_chebyshev_polynomial(ycoef, st, et, dt)
                        z = self.calculate_chebyshev_polynomial(zcoef, st, et, dt)
        ...
        # Assume the datetime is the same for all resolution.  Not true, but close
        # enough. This saves us from having slightly different solar angles for each
        # channel.
        gmd = self._get_geolocation_metadata(pro, xarray_obj.attrs)
        ...
        for band in dfs.keys():
            if band not in chlist.bands:
                ...
                # dfs[band] = {seg: df.decompress(outdir) for seg, df in dfs[band].items()}  # NOQA
                skip_segs = []
                for seg, df in dfs[band].items():
                    if df is None:
                        LOG.error(
                            "FAILED READ ABOVE, SKIPPING TRYING TO DECOMPRESS FILE %s ",
                            "%s",
                            band,
                            seg,
                        )
        ...
        for chan in chlist.chans:
            ...
            if chan.band not in radiances:
                ...
                radiances[chan.band] = self.countsToRad(counts, slope, offset)
            if chan.type == "Rad":
            ...
            if chan.type == "Ref":
                ...
                datavars[adname][chan.name] = self.radToRef(
                    radiances[chan.band],
            ...
            if chan.type == "BT":
                ...
                datavars[adname][chan.name] = self.radToBT(
        ...
        bottomlat = gvars[adname]["latitude"][np.ma.where(gvars[adname]["latitude"])][
            -1
        ]
        ...
        for dsname in datavars.keys():
            if (
                ...
                satzen_mask_inds = (
                    gvars[dsname]["satellite_zenith_angle"]
                    > satellite_zenith_angle_cutoff
                )
            else:
                # Latitude variable is always defined, and the mask here should
                # correspond to data that is valid and on disk.
    ...
    def call(
        ...
        # 'call_single_time' as that will initially be fed one file at a time. If that
        # file is a HRV file, then this will result in an empty list and an error thrown
        # in 'get_top_level_metadata'.
        # See note 1 at top of module
        fnames = [fname for fname in fnames if not any(val in fname for val in ["HRV"])]
...
PLUGIN_CLASS = SeviriHritReaderPlugin
