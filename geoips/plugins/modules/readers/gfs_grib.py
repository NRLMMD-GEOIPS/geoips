...
class GfsGribReaderPlugin(BaseReaderPlugin):
    """GFS Grib reader plugin class."""
    ...
    def convert_uv_to_kts(self, xr_df):
        ...
        xr_df["windspeed_kts"] = xr.DataArray(
            sqrt_func((ucomp**2) + (vcomp**2)) * self.MS_TO_KTS, dims=wind_dims
        )
    ...
    def press_grb_to_xr(
        self, grb_obj, custom_pressure_name="unk_pres", coord_flag=False
    ):
        ...
        for i in tmp_vars:
            ...
            data_attrs = {k: tmp_data[k] for k in self.tmp_attrs}
        ...
        if unq_pres:
            varlist[custom_pressure_name] = xr.DataArray(
                upres, dims=(custom_pressure_name)
            )
        elif not coord_flag:
    ...
    def surf_grb_to_xr(self, grb_obj, coord_flag=False):
        ...
        for i in grb_obj:
            ...
            data_attrs = {k: i[k] for k in self.surf_attrs}
            if coord_flag:
    ...
    def read_atmos(self, filenames, coord_flag=False):
        """Read gfs files.
        ...
        for f in filenames:
            ...
            xr_pres = self.press_grb_to_xr(tmp_pres, coord_flag=coord_flag)

            xr_pres = self.convert_uv_to_kts(xr_pres)

            # Surface
            tmp_surf = pg_frame.select(name=self.surface_vars, typeOfLevel="surface")
            surface_xr = self.surf_grb_to_xr(tmp_surf, coord_flag=coord_flag)

            # Single Layer, is ok to considerd as surface (slab)
            tmp_sl = pg_frame.select(typeOfLevel="atmosphereSingleLayer")
            single_atm_xr = self.surf_grb_to_xr(tmp_sl, coord_flag=coord_flag)

            tmp_atmos = pg_frame.select(typeOfLevel="atmosphere")
            atmos_xr = self.surf_grb_to_xr(tmp_atmos, coord_flag=coord_flag)
    ...
    def read_wave(self, filenames, coord_flag=False):
    ...
    def call(
        ...
        if "gfs" in gfs_type:
            xr_array = self.read_atmos(fnames, coord_flag=coord_flag)
        elif "gfswave" in gfs_type:
            xr_array = self.read_wave(fnames, coord_flag=coord_flag)
...
PLUGIN_CLASS = GfsGribReaderPlugin
