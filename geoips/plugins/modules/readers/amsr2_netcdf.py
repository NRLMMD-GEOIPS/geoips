...
class Amsr2NetcdfReaderPlugin(BaseReaderPlugin):
    """Amsr2 Netcdf reader plugin class."""
    ...
    def read_amsr2_winds(self, wind_xarray):
    ...
    def read_amsr2_mbt(self, full_xarray, varname, time_array=None):
        """
        Reformat AMSR2 xarray object appropriately.
        ...
        * attributes: source_name, platform_name, data_provider,
        ...
        sub_xarray[self.varnames[varname]] = full_xarray[varname]
        sub_xarray[self.varnames[varname]].attrs["channel_number"] = self.chan_nums[
            varname
        ]
        ...
        # channels sub_xarray.attrs['sample_distance_km'] = 3.0
        ...
        for dim in sub_xarray.sizes.keys():
            if "low_rez" in dim:
                ...
                # all channels. sub_xarray.attrs['sample_distance_km'] = 7.0
                sub_xarray.attrs["sample_distance_km"] = 2.0
                sub_xarray.attrs["interpolation_radius_of_influence"] = 20000

        # See dictionaries above for appropriate land mask array locations for each
        # variable
        full_xarray["LandMask"] = xarray.DataArray(
            full_xarray[self.land_var[chanstr]].to_masked_array()[
                self.land_num[chanstr], :, :
            ],
        ...
        if time_array is None:
            ...
            LOG.info(
                ...
                sub_xarray[self.varnames[varname]].dims,
            )
            ...
            # Have to set it on the actual xarray so it becomes a xarray format time
            # series (otherwise if you set it directly to ts, it is a pandas format time
            # series, and expand_dims doesn't exist).
            ...
            tss = [
                curr_time_array
                for ii in range(0, sub_xarray[self.varnames[varname]].shape[1])
            ]
            LOG.info("    Setting time DataArray")
    ...
    def read_amsr2_data(self, full_xarray, chans):
            ...
            if "Brightness" in varname:
                ...
                for xra in list(xarrays.values()):
                    if xra.time.dims == full_xarray[varname].dims:
                        usetime = xra.time
                new_xarray = self.read_amsr2_mbt(full_xarray, varname, usetime)
    ...
    def call(
        ...
        for fname in fnames:
            ...
            if hasattr(full_xarray, "title") and "AMSR2_OCEAN" in full_xarray.title:
                xarrays = self.read_amsr2_winds(full_xarray)

            elif hasattr(full_xarray, "title") and "MBT" in full_xarray.title:
                xarrays = self.read_amsr2_data(full_xarray, chans)
...
PLUGIN_CLASS = Amsr2NetcdfReaderPlugin
