...
class WindbarbsDmwAlgorithmPlugin(BaseAlgorithmPlugin):
    """Windbarbs DMW algorithm plugin class."""
    ...
    def _pre_call(self, data=None, *args, _obp_initiated=False, **kwargs):
        ...
        (``BaseAlgorithmPlugin._pre_call``).  Logs the resolved variable
        names at INTERACTIVE level so that ``var_map`` mismatches are
        immediately visible without needing a debugger.

        SSP runs pass ``xobj`` as a keyword with ``data=None``, so
        ...
        if _obp_initiated and isinstance(data, xr.Dataset):
            ...
            spd_name = var_map.get("speed", "wind_speed")
    ...
    def call(
        ...
        pressure_level_range : list of float, default=None
            * list of min and max pressure levels to filter derived motion wind
              retrievals.
        ...
        # validate the need of var_map.
        # Windbarbs dataset has wind_speed, wind_direction, and pressure
...
PLUGIN_CLASS = WindbarbsDmwAlgorithmPlugin
