...
class WindspeedAwips2FormatterOutputFormatterPlugin(BaseOutputFormatterPlugin):
    ...
    def call(
        ...
        if xarray_dict["METADATA"].attrs["source_name"] == "smap-spd":
            success_outputs = self.write_smap_awips2(
                xarray_dict, working_dir, utc_date_format
            )
        elif xarray_dict["METADATA"].attrs["source_name"] == "smos-spd":
            success_outputs = self.write_smos_awips2(
                xarray_dict, working_dir, utc_date_format
            )
        else:
            LOG.warning(
    ...
    def write_smap_awips2(self, xarray_dict, working_dir, utc_date_format):
        ...
        mintime, maxtime = self.get_min_max_times_from_two_nodes(
        ...
        for section_index in range(sections_count):
            ...
            asc_mask = self.make_smap_data_mask(asc_times, split_start, split_end)
            new_smap_data[asc_mask] = asc_speeds[asc_mask]

            des_mask = self.make_smap_data_mask(des_times, split_start, split_end)
            ...
            new_xarray_dict = self.make_smap_dataset(
                latitude,
                longitude,
                new_smap_data,
                timestamp_data,
    ...
    def make_smap_data_mask(self, times, split_start, split_end):
    ...
    def write_smos_awips2(self, xarray_dict, working_dir, utc_date_format):
        ...
        new_xarray_dict = self.make_smos_dataset(
            latitude,
            longitude,
            new_smos_data,
            new_time_data,
        ...
        new_xarray_dict.attrs["time_coverage_start"] = dataset.attrs[
            "time_coverage_start"
        ]
    ...
    def make_smos_dataset(self, lat, lon, smos_data, timestamp_data):
...
PLUGIN_CLASS = WindspeedAwips2FormatterOutputFormatterPlugin
