...
class PidLog:
    ...
    * Requires psutil and threading
    ...
    def print_mem_usg(
        self, logstr="", verbose=False, write_to_json=False, metadata=None
    ):
        ...
        usage_dict["memusg_highest"] = highest
        if write_to_json:
            # Store overall processing statistics to a json table
            json_data = usage_dict.copy()
            # The "process_times" key in the metadata dictionary passed to this function
            # holds a combination of the overall start and end times, along with the
            # timing statistics for each processed sector. The output json table will
            # only have one entry, so we need to condense the sector names and timings
            # into a semi-colon delimited list. The following loop separates the overall
            # start/end times of the procflow, and create a list of processed sector
            # names and sector times.
            proc_sectors, sector_timings = [], []
            for key, val in metadata["process_times"].items():
                if key == "overall_start":
                    json_data["starting_time"] = val
                elif key == "overall_end":
                    json_data["ending_time"] = val
                else:
                    proc_sectors.append(key)
                    try:
                        sector_timings.append(
                            (val["end"] - val["start"]).total_seconds()
                        )
                    except KeyError:
                        sector_timings.append(-9999)
            # After looping through the "process_times" dictionary, join the list of
            # sector names and processing times as a semi-colon delimited string
            json_data["proc_sector_names"] = ";".join(proc_sectors)
            json_data["proc_sector_times"] = ";".join([str(x) for x in sector_timings])
            # Now add the rest of the metadata to the output json dictionary
            for key, val in metadata.items():
                if key == "process_times":
                    continue
                json_data[key] = [val]
            # Creating a table where the columns are sorted alphabetically
            sorted_table = {x: json_data[x] for x in sorted(json_data.keys())}
            # Now create a pandas dataframe to easily dump it to a JSON file
            overall_start_datetime = metadata["process_times"]["overall_start"]
            output_directory = Path(gpaths["GEOIPS_OUTDIRS"]).joinpath(
                "processing_statistics",
                metadata["geoips_version"],
                metadata["procflow"],
                "overall",
                overall_start_datetime.strftime("%Y%m/%Y%m%d/%Y%m%d.%H"),
            )
            output_directory.mkdir(parents=True, exist_ok=True)
            output_json_file_name = ".".join(
                [
                    metadata["platform"],
                    metadata["source"],
                    metadata["sector_type"],
                    Path(metadata["output_config"]).name.replace(".yaml", ""),
                    str(metadata["procflow_id"]),
                    "json",
                ]
            )
            output_json_file = output_directory.joinpath(output_json_file_name)
            pd.DataFrame(sorted_table, index=[0]).to_json(output_json_file)
            LOG.info("Created: %s", output_json_file)
    ...
    def checkpoint_usage_stats(self, write_to_json=False, metadata=None):
        ...
        if write_to_json:
            ...
            for checkpoint_name, stats in checkpoint_usage_stats.items():
                for stat, val in stats.items():
                    table_dict[stat].append(val)
                for addtl_meta_key, addtl_meta_val in metadata.items():
                    table_dict[addtl_meta_key].append(addtl_meta_val)
                table_dict["checkpoint_name"] = checkpoint_name
            # Always sort the table by keys in alphabetical version
            # This will ensure the column order is consistent in the output JSON files
            sorted_table = {x: table_dict[x] for x in sorted(table_dict.keys())}
            # Now create a pandas dataframe to easily dump it to a JSON file
            overall_start_datetime = metadata["process_times"]["overall_start"]
            output_directory = Path(gpaths["GEOIPS_OUTDIRS"]).joinpath(
                "processing_statistics",
            ...
            output_json_file = output_directory.joinpath(output_json_file_name)
            pd.DataFrame(sorted_table).to_json(output_json_file)
            LOG.info("Created: %s", output_json_file)
