...
def get_source_name(xobjs):
    """Retrieve the source name from the metadata provided in xobjs.

    Where xobjs is a dictionary of xarray objects which contain only metadata for the
    data which will be processed.

    Parameters
    ----------
    xobjs: dict of xarray Datasets
        - Dictionary of xarray Datasets which contain metadata about the data currently
          being used in processing.

    Returns
    -------
    source_name: str
        - The source name in which the data is coming from
    """
