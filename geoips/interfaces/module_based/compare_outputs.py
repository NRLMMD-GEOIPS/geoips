...
"""Compare Outputs interface module."""
...
class CompareOutputsBase(BaseModulePlugin):
    """Compare Outputs routines to apply when comparing data outputs."""
    ...
    def images_match(self, output_product, compare_product, fuzz="5%"):
    ...
    def geoips_netcdf_match(self, output_product, compare_product):
    ...
    def text_match(self, output_product, compare_product):
    ...
    def test_product(self, output_product, compare_product, goodcomps, badcomps,
                     compare_strings):
        ...
        """
        matched_one = False
        if self.is_image(output_product):
            matched_one = True
            compare_strings += ["IMAGE "]
            if self.images_match(output_product, compare_product):
                goodcomps += ["IMAGE {0}".format(output_product)]
            else:
                badcomps += ["IMAGE {0}".format(output_product)]

        if self.is_geotiff(output_product):
            matched_one = True
            compare_strings += ["GEOTIFF "]
            if self.geotiffs_match(output_product, compare_product):
                goodcomps += ["GEOTIFF {0}".format(output_product)]
            else:
                badcomps += ["GEOTIFF {0}".format(output_product)]

        if self.is_text(output_product):
            matched_one = True
            compare_strings += ["TEXT "]
            if self.text_match(output_product, compare_product):
                goodcomps += ["TEXT {0}".format(output_product)]
            else:
                badcomps += ["TEXT {0}".format(output_product)]

        if self.is_geoips_netcdf(output_product):
            matched_one = True
            compare_strings += ["GEOIPS NETCDF "]
            if self.geoips_netcdf_match(output_product, compare_product):
    ...
    def compare_outputs(self, compare_path, output_products, test_product_func=None):
        ...
        for output_product in output_products:
            ...
            if basename(output_product) in compare_basenames:
                if test_product_func is None:
                    test_product_func = self.test_product
        ...
        for compare_product in glob(compare_path + "/*"):
            if (
                isfile(compare_product) and basename(compare_product) not
                in product_basenames
            ):
...
class CompareOutputsInterface(BaseModuleInterface):
    """Compare Outputs routines to apply when comparing data outputs."""
    ...
    plugin_class = CompareOutputsBase


compare_outputs = CompareOutputsInterface()
