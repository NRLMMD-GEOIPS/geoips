    ...
    if list_vars[0] not in xarray_atms.variables.keys():
        ...
        for i in range(len(list_vars)):
            if list_vars[i] == 'time_scan':
                ...
                merged_array = numpy.vstack([xarray_atms['timestamp'].to_masked_array(),
...
def atms_hdf5(fnames, metadata_only=False, chans=None, area_def=None, self_register=False):
    ...
    if metadata_only:                      # read-in datafiles first time if 'metadata_only= True'
        ...
        xarray_atms.attrs['platform_name'] = fileobj.attrs['Platform_Short_Name'][0,
                                                                                  0].decode("utf-8")  # could be changed if needed
