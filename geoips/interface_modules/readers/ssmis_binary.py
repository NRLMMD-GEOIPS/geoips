...
#           Process LAS data record
        ...
        for ii in range(nscan_las):
            ...
            for jj in range(scenecounts_las[ii]):
                try:
                    las_lat, las_lon, las_ch01_3x3, las_ch02_3x3, las_ch03_3x3, las_ch04_3x3, las_ch05_3x3, las_ch06_3x3, las_ch07_3x3, las_ch08_5x5, las_ch09_5x5, las_ch10_5x5, las_ch11_5x5, las_ch18_5x5, las_ch24_3x3, las_height_1000mb, las_surf = np.fromstring(
...
# set xarray object for imager variables
    ...
    xarray_imager['timestamp'] = xr.DataArray(
        pd.DataFrame(time_imager).astype(int).apply(
...
# set xarray object for enviro  variables
    ...
    xarray_enviro['timestamp'] = xr.DataArray(
        pd.DataFrame(time_enviro).astype(int).apply(
            pd.to_datetime, format='%Y%j%H%M'))
