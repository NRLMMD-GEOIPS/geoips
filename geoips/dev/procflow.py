...
def is_valid_procflow(procflow_func_name):
    ...
    Return values should be 0 for successful completion,
    non-zero otherwise - return values are not verified within this
    format check.
    ...
    Parameters
    ----------
    procflow_func_name : str
        Desired procflow function
        (ie, ``single_source``, ``pmw_tbs``, ``stitched_procflow``, etc)

    Returns
    -------
    is_valid : bool
        * True if ``procflow_func_name`` has the appropriate call signature
    ...
    ..
        'standard' :
            call signature <procflow_func_name>(fnames, command_line_args=None)
            return value: 0 for successful completion, non-zero for failure.
            This can be used for determining successful output
            comparisons for unit tests / CI/CD.
    """
    ...
    try:
        ...
        if "procflow_type" in str(resp):
            LOG.warning(
                f"Module {procflow_func_name} in procflow package not a valid procflow module: "
                f'If this is intended to be a valid procflow module, ensure "{type_name}" is defined:'
                f'Exception: "{resp}"'
            )
        else:
            LOG.exception(
                f'ImportError in module {procflow_func_name} in procflow package. Exception: "{resp}"'
            )
            raise
        return False
    try:
    ...
    if procflow_type not in required_args:
        raise TypeError(
            f"INVALID PROCFLOW FUNC {procflow_func_name}:\n"
    ...
    if not set(required_args[procflow_type]).issubset(set(procflow_args)):
        LOG.error(
            "INVALID READER '%s': '%s' procflow type must have required arguments: '%s'",
            procflow_func_name,
            procflow_type,
            required_args[procflow_type],
        )
        return False
    ...
    if not set(required_kwargs[procflow_type]).issubset(set(procflow_kwargs)):
        LOG.error(
            "INVALID READER '%s': '%s' procflow type must have optional kwargs: '%s'",
            procflow_func_name,
            procflow_type,
            required_kwargs[procflow_type],
        )
        return False
...
def get_procflow(procflow_func_name):
    ...
    See: geoips.dev.procflow.is_valid_procflow for full list of
    supported procflow function call signatures and return values
...
def get_procflow_type(procflow_func_name):
    ...
    ----------
    procflow_func_name : str
        Desired procflow module (ie, 'single_source', 'stitched_procflow', etc)

    Returns
    -------
    procflow_type : str
        Procflow type currently found in
        ``<geoips_package>.procflows.<procflow_func_name>.procflow_type``
...
def list_procflows_by_type():
    ...
    List all available procflows within the current GeoIPS instantiation,
    sorted by procflow_type

    Driver "type" determines exact required call signatures and return values

    See geoips.dev.procflow.is_valid_procflow? for a list of available
        procflow types and associated call signatures / return values.
    Use geoips.dev.procflow.get_procflow(procflow_func_name)
        to retrieve the requested procflow

    Returns
    -------
    dict
        Dictionary with all procflow types as keys, and associated
        procflow names (str) as values.
    """
...
def test_procflow_interface():
    ...
    Finds and opens every procflow func available within the current
    geoips instantiation

    See geoips.dev.procflow.is_valid_procflow? for a list of available
        procflow func types and associated call signatures / return values.
    See geoips.dev.procflow.get_procflow(procflow_func_name)
        to retrieve the requested procflow func

    Returns
    -------
    list
        List of all successfully opened geoips procflow funcs
    """
