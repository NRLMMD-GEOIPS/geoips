...
def is_valid_title(title_func_name):
    ...
    Return values should be as specified below, but return values are
    not verified with this format check.

    Parameters
    ----------
    title_func_name : str
        Name of requested title function

    Returns
    -------
    bool
        * True if 'title_func_name' function has the appropriate call signature
    ...
    try:
        ...
        LOG.warning(
            "Module %s in title package not a valid title module: "
    ...
    try:
        ...
        raise ImportError(
            f"INVALID TITLE {title_func_name}: "
    ...
    if title_type not in required_args:
        raise TypeError(
            f"INVALID TITLE FUNC {title_func_name}: "
    ...
    if not set(required_args[title_type]).issubset(set(title_args)):
        LOG.error(
            "INVALID TITLE FUNCTION '%s': '%s' title type must have required arguments: '%s'",
            title_func_name,
            title_type,
            required_args[title_type],
        )
        return False
    ...
    if not set(required_kwargs[title_type]).issubset(set(title_kwargs)):
        LOG.error(
            "INVALID TITLE FUNCTION '%s': '%s' title type must have optional kwargs: '%s'",
            title_func_name,
            title_type,
            required_kwargs[title_type],
        )
        return False
...
def get_remove_duplicates_func(title_func_name):
    ...
    -------
    function
        Function for generating titles of the specified format.

    See Also
    --------
    ``geoips.dev.title.is_valid_title``
        full list of supported title function call signatures and return values
    """
    curr_func = find_entry_point("title_formats", title_func_name)
    return getattr(
        import_module(curr_func.__module__), title_func_name + "_remove_duplicates"
    )
...
def get_title_type(func_name):
    ...
    -------
    str
        Type of requested title function
...
def list_titles_by_type():
    ...
    List all available title format functions within the
    current GeoIPS instantiation, sorted by title_type

    title function "type" determines exact required call signatures and
    return values

    See geoips.dev.title.is_valid_title? for a list of available title
    format types and
    associated call signatures / return values.
    See geoips.dev.title.get_title(title_func_name) to retrieve the requested
    title format function

    Returns
    -------
    dict
        Dictionary with all title format types as keys,
...
def test_title_interface():
    ...
    Finds and opens every title func available within the current
    geoips instantiation

    Returns
    -------
    list
        List of all successfully opened geoips title funcs

    See Also
    --------
    ``geoips.dev.title.is_valid_title``
        full list of supported title function call signatures and return values
    ``geoips.dev.title.get_title(title_func_name)``
        retrieve the requested title func
    """
