...
def is_valid_outputter(output_func_name):
    ...
    Parameters
    ----------
    output_func_name : str
        Name of requested output product function
        (ie, 'imagery_annotated', 'unprojected_image', etc)

    Returns
    -------
    bool
        * True if 'output_func_name' function has the appropriate call signature
    ...
    required_args = {
        ...
        # No outfnames arg implies the output filename list will *not* be checked
        # for consistency
        "xrdict_area_product_to_outlist": ["xarray_dict", "area_def", "product_name"],
    }
    ...
    try:
        ...
        LOG.warning(
            f'Module {output_func_name} in output package not a valid output module: If this is intended to be a valid output module, ensure "output_type" is defined: Exception: "{resp}"'
        )
        return False
    try:
    ...
    if output_type not in required_args:
        raise TypeError(
            f"INVALID OUTPUT FUNC {output_func_name}:\n"
    ...
    if not set(required_args[output_type]).issubset(set(output_args)):
        LOG.error(
            "INVALID OUTPUT FUNCTION '%s': '%s' output type must have required arguments: '%s'",
            output_func_name,
            output_type,
            required_args[output_type],
        )
        return False
    ...
    if not set(required_kwargs[output_type]).issubset(set(output_kwargs)):
        LOG.error(
            "INVALID OUTPUT FUNCTION '%s': '%s' output type must have optional kwargs: '%s'",
            output_func_name,
            output_type,
            required_kwargs[output_type],
        )
        return False
...
def get_outputter(output_func_name):
    ...
    ----------
    output_func_name : str
        Name of requested output product function, ie:

        * ``create_standard_imagery``
        * ``create_standard_metoctiff``
        * ``create_standard_windbarbs``

    Returns
    -------
    <output function>
        Function for generating output products of the specified format.

    See Also
    --------
    ::

        See: ``geoips.dev.output.is_valid_outputter``
        for full list of supported output function call signatures
        and return values
    """
...
def get_outputter_type(output_func_name):
    ...
    -------
    outputter_type : str
        Type of requested output function
...
def list_outputters_by_type():
    ...
    List all available output format functions within the
    current GeoIPS instantiation, sorted by output_type
    ...
    -------
    all_outputters : dict
        Dictionary with all output types as keys, and
        associated output format function names (str) as values.

    See Also
    --------
    ``geoips.dev.output.is_valid_outputter?``
        list of available output format types and
        associated call signatures / return values.
    ``geoips.dev.output.get_outputter(output_name)``
        retrieve the requested output format function
    """
...
def test_output_interface():
    ...
    Finds and opens every output func available within the
    current geoips instantiation

    Returns
    -------
    list
        List of all successfully opened geoips output funcs

    See Also
    --------
    ``geoips.dev.output.is_valid_outputter?``
        list of available output func types and
        associated call signatures / return values.
    ``geoips.dev.output.get_outputter(output_func_name)``
        retrieve the requested output func
    """
