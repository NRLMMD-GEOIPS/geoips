...
"""Algorithm interface will be deprecated v2.0.

Wrapper functions for geoips algorithm interfacing.
...
def is_valid_alg(alg_func_name):
    ...
    ----------
    alg_func_name :str
        * Desired algorithm function
        * (ie, 'visir.IR_BD', 'pmw_tb.pmw_89pct', 'pmw_tb.color89', etc)

    Returns
    -------
    is_valid_alg : bool
        * True if 'alg_func_name' has the appropriate call signature
    ...
    Call signature array lists:
        Must be same length and in same order as required within function.
        Specified in product/sensor config.

    Return arrays:
        array_2d is a single 2d array of the processed dat
        array_rgba contains dstacked red, green, blue and (optional) alpha arrays.
        dstacked_array contains arbitrary output arrays dstacked into a single
        numpy array
    ...
    if not set(required_args[alg_func_type]).issubset(set(alg_func_args)):
        LOG.error(
            "INVALID ALG FUNC '%s': "
            "'%s' alg func type must have required arguments: '%s'",
            alg_func_name,
            alg_func_type,
            alg_func_args[alg_func_type],
        )
        return False
    ...
    if not set(required_kwargs[alg_func_type]).issubset(set(alg_func_kwargs)):
        LOG.error(
            "INVALID ALG FUNC '%s': '%s' alg_func type must have kwargs: '%s'",
            alg_func_name,
            alg_func_type,
            required_kwargs[alg_func_type],
        )
        return False
...
def get_alg(alg_func_name):
    ...
    See: geoips.dev.alg.is_valid_alg for full list of supported algorithm
...
def list_algs_by_type():
    ...
    List all available algorithm functions within the current GeoIPS
    instantiation, sorted by alg_type

    Algorithm function "type" determines exact required call signatures
    and return values

    See geoips.dev.alg.is_valid_alg? for a list of available algorithm function
        types and associated call signatures / return values.
    See geoips.dev.alg.get_alg(alg_func_name) to retrieve the requested
        algorithm function
    See geoips.dev.alg.get_alg_type(alg_func_name) to retrieve the requested
        algorithm function type

    Returns
    -------
    dict
        Dictionary with all algorithm function types as keys, and associated
        algorithm function names (str) as values.
    """
...
def test_alg_interface():
    ...
    Finds and opens every alg func available within the current geoips
    instantiation

    Returns
    -------
    list
        List of all successfully opened geoips alg funcs

    See Also
    --------
    * See geoips.dev.alg.is_valid_alg? for a list of available alg func types
