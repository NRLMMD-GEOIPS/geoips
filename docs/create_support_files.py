...
def create_version_config(
    ...
    config = [
        {"version": "stable", "url": f"{base_url}stable/", "is_latest": True},
        {"version": "dev", "url": f"{base_url}dev/"},
    ]
    # Now, add the directories, sorted by version
    for version_dir in sorted(versions, key=lambda v: Version(v[1:])):
        config.append(
            {
                "name": version_dir,
...
def write_versions_json(config: list[dict[str, str | bool]], output_path: Path):
...
def write_html_redirect(output_path: Path, which: str = "index"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-O", "--output-dir", type=Path, help="Output Path", required=True
    )
    parser.add_argument(
        "--base-url",
        type=str,
    ...
    # Validate inputs
    if (args.create_version_file or args.create_stable_symlink) and not hasattr(
        args, "scan_dir"
    ):
        raise ValueError(
            (
    ...
    if not args.base_url.strip():
        # if overwritten with empty, set back to default
        base_url = DEFAULT_BASE_URL
    elif not args.base_url.strip().endswith("/"):
        raise ValueError(
            f"--base-url must have a trailing slash to be valid, not {args.base_url}"
        )
    else:
    ...
    if args.create_version_file:
        if not len(version_dirs):
            ...
            config = {"version": "dev", "url": f"{base_url}dev/"}
        else:
            config = create_version_config(version_dirs, base_url)
    ...
    if args.create_stable_symlink:
        try:
            latest = sorted(version_dirs, key=lambda v: Version(v[1:]))[-1]
        except IndexError:
            raise ValueError(
                (
