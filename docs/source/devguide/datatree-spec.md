...
<!-- cspell:ignore eywords rallel -->
<!-- "eywords"/"rallel" are cspell allowCompoundWords artifacts on the correctly-spelled
     words "Keywords"/"Parallel"; they are not typos. -->
...
:::{note}
This is the full, RFC-style internal specification for the GeoIPS 2.0 DataTree runtime and Order-Based Processing. Most users only need the {ref}`DataTree data model <datatree>` and {ref}`scripting guide <scripting-guide>`.
:::
...
| `when` | MAY | expression | Skip step if expression is false. Expressions must be pandas-style filter expressions, not arbitrary Python code (e.g., `when: "{% raw %}{{ config.has_zenith }}{% endraw %}"`). |
