...
Kyle Hilburn works with GLM data, and has requested that we build a reader for that data
so he can implement his routines in GeoIPS. This branch has not only created that
reader, but also includes products, a feature_annotator, output_formatter, and a basic
algorithm to visualize GLM area products. We also modified mpl_utils.py to include a
patch adder, which adds patches to the appropriate figure in the correct Coordinate
Reference System (CRS), with the appropriate colormap applied.

::

    modified: geoips/geoips/image_utils/mpl_utils.py
    modified: geoips/geoips/plugins/modules/procflows/single_source.py
    modified: geoips/pyproject.toml
    added: geoips/geoips/plugins/modules/algorithms/glm_alg.py
    added: geoips/geoips/plugins/modules/output_formatters/shape_patches.py
    added: geoips/geoips/plugins/modules/readers/glm_netcdf.py
    added: geoips/geoips/plugins/yaml/products/glm.yaml
    added: geoips/tests/scripts/test_glm.sh
