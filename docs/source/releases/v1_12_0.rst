...
So far, we've yet to discover what exactly is occuring that prevents polar data from
being shown in imagery. It seems that the data is being interpolated into a manner that
cannot be plotted by MatPlotLib. To work around this, we can provide
``--no-presectoring``, which mitigates these problems for the time being.
