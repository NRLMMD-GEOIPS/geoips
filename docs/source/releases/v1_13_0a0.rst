...
Currently the CLI implements a command called ``geoips get``, which retrieves
information about a certain geoips artifact. We've found that ``get`` is a bit
ambiguous, as this command does not retrieve the artifact as an actual python object; it
moreso ``describes`` what the artifact is and some ancillary information related to it.

To reflect what this command does to a greater extent, we've decided to rename
``geoips get`` to ``geoips describe``. This is a refactoring change, but will not change
any of the functionality that was implemented for ``geoips get``.

NOTE: All renamed files were also modified to reference ``geoips describe`` rather than
``geoips get``.
...
::
    ...
    renamed: geoips/commandline/geoips_get.py -- geoips/commandline/geoips_describe.py
    renamed: tests/unit_tests/commandline/test_geoips_get_family.py -- tests/unit_tests/commandline/test_geoips_describe_family.py
    renamed: tests/unit_tests/commandline/test_geoips_get_interface.py -- tests/unit_tests/commandline/test_geoips_describe_interface.py
    renamed: tests/unit_tests/commandline/test_geoips_get_package.py -- tests/unit_tests/commandline/test_geoips_describe_package.py
    renamed: tests/unit_tests/commandline/test_geoips_get_plugin.py -- tests/unit_tests/commandline/test_geoips_describe_plugin.py