...
Instead of generating a plugin object via a class factory for a given module, we will
transform all module-based plugins to be true python classes. This accomplishes a few
things.

1. Transparency to the user/developer
2. Removes any 'black magic' related to how module-based plugins are transformed into a
   plugin object.
3. Clarifies the chain of inheritance and shared functionality among plugin objects.

Plugin Registry Impacts
-----------------------

In order to properly load any GeoIPS plugin, we make use of a plugin registry file. This
file contains metadata about every plugin which informs GeoIPS where all plugins are
located and what interface a given plugin falls under.

Previously, there were three top-level types for plugins. ``module-based, text-based,
and yaml-based``. In order to clarify the true type of module-based plugins, we've
changed their top-level type to ``class-based`` instead of ``module-based``. Even if a
plugin is truly ``module-based``, we store these plugins under the ``class-based`` key
since they are converted to a class-based plugin object at run time.

We've added an additional metadata field to capture this corner case. This new field is
titled ``is_derived_from_module`` and depicts whether or not a plugin was derived from a
module or is truly a class-based plugin. For all legacy module-based plugins, this field
will be set to true. This is later used via the plugin registry to determine which mode
it should use to load the requested plugin.
...
for the plugin registry to be able to determine which object is truly the class-based
plugin to collect metadata from. This variable is titled ``PLUGIN_CLASS`` and should
point to the uninstantiated version of your class-based plugin.

Below we'll demonstrate the differences between a module-based plugin and a class-based
plugin.

Module-Based ``single_channel`` Algorithm Plugin
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Class-Based ``single_channel`` Algorithm Plugin
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   ...
   class SingleChannelAlgorithmPlugin(BaseAlgorithmPlugin):
      ...
      def call(
         ...
         arrays : list of numpy.ndarray
               * list of numpy.ndarray or numpy.MaskedArray of channel data
               * MUST be length one for single_channel algorithm.
               * If applying solar zenith corrections and diurnal masking, the second index
               in the arrays list must be the solar zenith angle array. This is
               controlled in product YAML.
               * If applying a satellite zenith angle cutoff, the final index in the arrays
               list must be the satellite zenith angle array. This is controlled in the
               product YAML.
         output_data_range : list of float, default=None
